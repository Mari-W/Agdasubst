...
-- This file is EXPECTED TO FAIL.  `agda SemRewriteProbeJ.agda` → exit 42.
-- Round 7: + ⊛ᵀ-lift-wk, ⊛ᵀ-lift-fusion, ⊙ᵀ-lift-{zero,suc,wk,fusion,ren}; 21 rules.
-- Measured: 8 non-joinable pairs (27 → 8 in one pass). Residuals R1–R8: `⊛ᵀ-cons` is a bad member (strat omits renaming `distributivity`), and strat's mixed `⟨⟩-lift-*` family is not yet mirrored.
-- Nothing imports this file.  It exists so that §7 of
-- REPORT-adequacy.md is reproducible.
-- ROUND 7: +⊛ᵀ-lift-wk, ⊛ᵀ-lift-fusion, ⊙ᵀ-lift-{zero,suc,wk,fusion,ren}.
-- Measured: 8 pairs (27→8). Residuals: ⊛ᵀ-cons is a bad member (strat omits renaming distributivity) + strat's mixed ⟨⟩-lift-* family not yet mirrored.
-- Evidence for §7 of REPORT-adequacy.md.  Nothing imports it.
-- ⚠ MEASUREMENT PROBE for REPORT-adequacy.md §7 ⚠
-- ROUND 7.  Round 6 residuals named these companions; adding them.
-- ROUND 6 was:  Configuration FIXED at round 4's (INWARD orientation).
...
opaque
  ...
  -- ★ ROUND 7 CLOSERS for the ⊛ᵀ-assoc family ★
  -- ≈ `distributivity  (closes ⊛ᵀ-assoc vs `interact, `interact-⨟)
  ⊛ᵀ-cons : ∀ {Δ₁ Δ₂ l} (α : Δ₂ ∋ˡ l) (ζ : Ren Δ₁ Δ₂) (η : Env* Δ₂) →
            ⊛ᵀ (α ∙ᴿ ζ) η ≡ ext (lookupᵀ α η) (⊛ᵀ ζ η)
  ⊛ᵀ-cons α ζ η = refl

  -- ≈ `lift-wk  (closes ⊛ᵀ-assoc vs `lift-wk, `lift-wk-⨟)
  ⊛ᵀ-lift-wk : ∀ {Δ₁ Δ₂ l} (ζ : Ren Δ₁ Δ₂) (η : Env* (l ∙ Δ₂)) →
               ⊛ᵀ wkᴿ (⊛ᵀ (ζ ↑ᴿ) η) ≡ ⊛ᵀ ζ (⊛ᵀ wkᴿ η)
  ⊛ᵀ-lift-wk ζ η = trans (sym (⊛ᵀ-assoc wkᴿ (ζ ↑ᴿ) η)) (⊛ᵀ-assoc ζ wkᴿ η)

  -- ≈ `lift-fusion  (closes ⊛ᵀ-assoc vs `lift-fusion, `lift-fusion-⨟)
  ⊛ᵀ-lift-fusion : ∀ {Δ₁ Δ₂ Δ₃ l} (ζ₁ : Ren Δ₁ Δ₂) (ζ₂ : Ren Δ₂ Δ₃) (η : Env* (l ∙ Δ₃)) →
                   ⊛ᵀ (ζ₁ ↑ᴿ) (⊛ᵀ (ζ₂ ↑ᴿ) η) ≡ ⊛ᵀ ((ζ₁ ⨟ᴿ ζ₂) ↑ᴿ) η
  ⊛ᵀ-lift-fusion ζ₁ ζ₂ η = sym (⊛ᵀ-assoc (ζ₁ ↑ᴿ) (ζ₂ ↑ᴿ) η)
...
opaque
  ...
  -- ★ ROUND 7 CLOSERS for the lkp-⊙ / ⊙ᵀ-assoc families ★
  -- ≈ beta-lift-zero
  ⊙ᵀ-lift-zero : ∀ {Δ₁ Δ₂ l} (σ : Sub Δ₁ Δ₂) (η : Env* (l ∙ Δ₂)) →
                 lookupᵀ here (⊙ᵀ (σ ↑ˢ) η) ≡ lookupᵀ here η
  ⊙ᵀ-lift-zero σ η = refl

  -- ≈ beta-lift-suc
  ⊙ᵀ-lift-suc : ∀ {Δ₁ Δ₂ l l′} (α : Δ₁ ∋ˡ l) (σ : Sub Δ₁ Δ₂) (η : Env* (l′ ∙ Δ₂)) →
                lookupᵀ (there α) (⊙ᵀ (σ ↑ˢ) η) ≡ lookupᵀ α (⊙ᵀ σ (⊛ᵀ wkᴿ η))
  ⊙ᵀ-lift-suc α σ η =
    cong (lookupᵀ α) (trans (⊙ᵀ-assoc σ ⟨ wkᴿ ⟩ η) (cong (⊙ᵀ σ) (⊙ᵀ-⟨⟩ wkᴿ η)))

  -- ≈ lift-wk
  ⊙ᵀ-lift-wk : ∀ {Δ₁ Δ₂ l} (σ : Sub Δ₁ Δ₂) (η : Env* (l ∙ Δ₂)) →
               ⊛ᵀ wkᴿ (⊙ᵀ (σ ↑ˢ) η) ≡ ⊙ᵀ σ (⊛ᵀ wkᴿ η)
  ⊙ᵀ-lift-wk σ η =
    trans (sym (⊙ᵀ-⟨⟩ wkᴿ (⊙ᵀ (σ ↑ˢ) η)))
    (trans (sym (⊙ᵀ-assoc ⟨ wkᴿ ⟩ (σ ↑ˢ) η))
    (trans (⊙ᵀ-assoc σ ⟨ wkᴿ ⟩ η)
           (cong (⊙ᵀ σ) (⊙ᵀ-⟨⟩ wkᴿ η))))

  -- ≈ lift-fusion
  ⊙ᵀ-lift-fusion : ∀ {Δ₁ Δ₂ Δ₃ l} (σ₁ : Sub Δ₁ Δ₂) (σ₂ : Sub Δ₂ Δ₃) (η : Env* (l ∙ Δ₃)) →
                   ⊙ᵀ (σ₁ ↑ˢ) (⊙ᵀ (σ₂ ↑ˢ) η) ≡ ⊙ᵀ ((σ₁ ⨟ˢ σ₂) ↑ˢ) η
  ⊙ᵀ-lift-fusion σ₁ σ₂ η = sym (⊙ᵀ-assoc (σ₁ ↑ˢ) (σ₂ ↑ˢ) η)

  -- ≈ beta-lift-ren-↑
  ⊙ᵀ-lift-ren : ∀ {Δ₁ Δ₂ Δ₃ l l′} (α : (l′ ∙ Δ₁) ∋ˡ l) (ζ : Ren Δ₁ Δ₂)
                (σ : Sub Δ₂ Δ₃) (η : Env* (l′ ∙ Δ₃)) →
                lookupᵀ (α &ᴿ (ζ ↑ᴿ)) (⊙ᵀ (σ ↑ˢ) η)
                ≡ lookupᵀ α (⊙ᵀ ((⟨ ζ ⟩ ⨟ˢ σ) ↑ˢ) η)
  ⊙ᵀ-lift-ren here      ζ σ η = refl
  ⊙ᵀ-lift-ren (there α) ζ σ η =
    trans (⊙ᵀ-lift-suc (α &ᴿ ζ) σ η)
    (sym (trans (⊙ᵀ-lift-suc α (⟨ ζ ⟩ ⨟ˢ σ) η)
         (trans (cong (lookupᵀ α) (⊙ᵀ-assoc ⟨ ζ ⟩ σ (⊛ᵀ wkᴿ η)))
         (trans (cong (lookupᵀ α) (⊙ᵀ-⟨⟩ ζ (⊙ᵀ σ (⊛ᵀ wkᴿ η))))
                (lkp-⊛ α ζ (⊙ᵀ σ (⊛ᵀ wkᴿ η)))))))

{-# REWRITE
  lkp-ext-zero lkp-ext-suc
  lkp-⊛ lkp-lift-ext ⊛ᵀ-lift-ext ⊛ᵀ-interact ⊛ᵀ-id ⊛ᵀ-assoc ⟦⟧ᵀ-ren
  ⊛ᵀ-cons ⊛ᵀ-lift-wk ⊛ᵀ-lift-fusion
  lkp-⊙ ⊙ᵀ-⟨⟩ ⊙ᵀ-cons ⊙ᵀ-lift-ext ⊙ᵀ-id ⊙ᵀ-assoc ⟦⟧ᵀ-sub
  ⊙ᵀ-lift-zero ⊙ᵀ-lift-suc ⊙ᵀ-lift-wk ⊙ᵀ-lift-fusion ⊙ᵀ-lift-ren
#-}
