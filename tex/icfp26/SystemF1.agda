...
opaque
  ...
  coincidence-lemma₄ = fun-ext λ { zero → refl; (suc x) → refl }
  coincidence-lemma₅ : (σ₁ ⨟ (⟨ wk ⟩ ⨟ ⟨ zero ∙ᴿ (ρ₂ ∘ wk) ⟩)) ≡ (σ₁ ⨟ (⟨ ρ₂ ⟩ ⨟ ⟨ wk ⟩))
  coincidence-lemma₅ = fun-ext λ { zero → refl; (suc x) → refl }
  coincidence-lemma₆ : (⟨ wk ∘ (zero ∙ᴿ (ρ₂ ∘ wk)) ⟩ ⨟ ((T′ ⋯ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩)) ∙ˢ ⟨ idᴿ ⟩)) ≡ ⟨ ρ₂ ⟩ 
  coincidence-lemma₆ = fun-ext λ { zero → refl; (suc x) → refl }
  coincidence-lemma₇ : ((T′ ⋯ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩)) ∙ˢ (σ₁ ⨟ (⟨ ρ₂ ∘ wk ⟩ ⨟ ((T′ ⋯ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩)) ∙ˢ ⟨ idᴿ ⟩))))≡ ((T′ ⋯ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩)) ∙ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩))
  coincidence-lemma₇ = fun-ext λ { zero → refl; (suc x) → refl }
...
_∣_∙ˢ*_ : ∀ σ T → σ ∣ Γ₁ ⇒ˢ Γ₂ → (T ∙ˢ σ) ∣ (Γ₁ ▷*) ⇒ˢ Γ₂
(_ ∣ T ∙ˢ* Σ) _ (suc* x) = Σ _ x -- no subst swap wk single subst

_∣_⇑ˢ_ : ∀ σ → σ ∣ Γ₁ ⇒ˢ Γ₂ → ∀ T → σ ∣ (Γ₁ ▷ T) ⇒ˢ (Γ₂ ▷ (T ⋯ˢ σ))
σ ∣ Σ ⇑ˢ T = σ ∣ (` zero) ∙ˢ λ _ x → idᴿ ∣ (Σ _ x) ⋯ᴿ (Wk _) -- no subst swap sub wk
...
opaque
  ...
  Identityᵣ (` x)      = refl
  Identityᵣ (λx e)     = cong λx_ (trans (cong (⟨ idᴿ ⟩ ∣ e ⋯ˢ_) η-Id) (Identityᵣ e))
  Identityᵣ (Λα e)     = cong Λα_ (trans (cong (⟨ idᴿ ⟩ ∣ e ⋯ˢ_) η*-Id) (Identityᵣ e))
  ...
  η-Idᴿᵣ : idᴿ ∣ (zero {Γ = Γ} {T = T}) ∙ᴿ (Wk T) ≡ (Id {Γ = Γ ▷ T})
  η-Idᴿᵣ = fun-ext λ _ → fun-ext λ { zero → refl; (suc x) → refl }

  η*-Idᴿᵣ : idᴿ ∣ (Id {Γ = Γ}) ↑ᴿ* ≡ Id
...
lift*-dist-Compˢᴿ  σ₁ ρ₂ Σ₁ Ρ₂ = fun-ext λ _ → fun-ext λ 
  { (suc* x) → 
    let e = Σ₁ _ x in begin 
    _ ≡⟨ Coincidence (wk ∣ e ⋯ᴿ wk*) (ρ₂ ∣ Ρ₂ ↑ᴿ*) ⟩ 
    _ ≡⟨ Composeᴿᴿ e _ _ wk* (ρ₂ ∣ Ρ₂ ↑ᴿ*)  ⟩ 
    _ ≡⟨ sym (Composeᴿᴿ e _ _ Ρ₂ wk*) ⟩ 
    _ ≡⟨ cong (wk ∣_⋯ᴿ wk*) (sym (Coincidence e _)) ⟩ 
    _ ∎ } 
    
Composeˢᴿ : ∀ (e : Expr Γ₁ T) (σ₁ : n₁ →ˢ n₂) (ρ₂ : n₂ →ᴿ n₃) (Σ₁ : σ₁ ∣ Γ₁ ⇒ˢ Γ₂) (Ρ₂ : ρ₂ ∣ Γ₂ ⇒ᴿ Γ₃) →
  ρ₂ ∣ (σ₁ ∣ e ⋯ˢ Σ₁) ⋯ᴿ Ρ₂ ≡ (σ₁ ⨟ ⟨ ρ₂ ⟩) ∣ e ⋯ˢ (σ₁ , ⟨ ρ₂ ⟩ ∣ Σ₁ ⨾ (ρ₂ ∣⟪ Ρ₂ ⟫))
Composeˢᴿ (` x)     _  _  Σ₁  _  = sym (Coincidence (Σ₁ _ x) _)
Composeˢᴿ (λx e)    σ₁ ρ₂ Σ₁ Ρ₂  = cong λx_ (trans (Composeˢᴿ e _ _ _ _) (cong ((σ₁ ⨟ ⟨ ρ₂ ⟩) ∣ e ⋯ˢ_) (Lift-Dist-Compˢᴿ Σ₁ Ρ₂)))
Composeˢᴿ (Λα e)    σ₁ ρ₂ Σ₁ Ρ₂  = cong Λα_ (trans (Composeˢᴿ e _ _ _ _) (cong (((σ₁ ⨟ ⟨ ρ₂ ⟩) ↑ˢ) ∣ e ⋯ˢ_) (lift*-dist-Compˢᴿ σ₁ ρ₂ Σ₁ Ρ₂)))
Composeˢᴿ (e₁ · e₂) _  _  _  _   = cong₂ _·_ (Composeˢᴿ e₁ _ _ _ _) (Composeˢᴿ e₂ _ _ _ _)
Composeˢᴿ (e ·* T′) σ₁ ρ₂ Σ₁ Ρ₂  = cong (_·* (T′ ⋯ˢ (σ₁ ⨟ ⟨ ρ₂ ⟩))) (Composeˢᴿ e σ₁ ρ₂ Σ₁ Ρ₂) 


Lift-Dist-Compˢˢ : (Σ₁ : σ₁ ∣ Γ₁ ⇒ˢ Γ₂) (Σ₂ : σ₂ ∣ Γ₂ ⇒ˢ Γ₃) →
  σ₁ , σ₂ ∣ (σ₁ ∣ Σ₁ ⇑ˢ T) ⨾ (σ₂ ∣ Σ₂ ⇑ˢ (T ⋯ˢ σ₁)) ≡ ((σ₁ ⨟ σ₂) ∣ (σ₁ , σ₂ ∣ Σ₁ ⨾ Σ₂) ⇑ˢ T)
...
lift*-dist-Compˢˢ : (σ₁ : n₁ →ˢ n₂) (σ₂ : n₂ →ˢ n₃) (Σ₁ : σ₁ ∣ Γ₁ ⇒ˢ Γ₂) (Σ₂ : σ₂ ∣ Γ₂ ⇒ˢ Γ₃) →
  (σ₁ ↑ˢ) , (σ₂ ↑ˢ) ∣ (σ₁ ∣ Σ₁ ↑ˢ*) ⨾ (σ₂ ∣ Σ₂ ↑ˢ*) ≡ ((σ₁ ⨟ σ₂) ∣ (σ₁ , σ₂ ∣ Σ₁ ⨾ Σ₂) ↑ˢ*)
lift*-dist-Compˢˢ _ σ₂ Σ₁ Σ₂ = fun-ext λ _ → fun-ext λ 
  { (suc* x) → 
    let e = Σ₁ _ x in begin
    _ ≡⟨ Composeᴿˢ e _ _ _ _ ⟩
    _ ≡⟨ cong ((σ₂ ⨟ ⟨ wk ⟩) ∣ e ⋯ˢ_) (fun-ext (λ _ → fun-ext λ { x → sym (Coincidence (Σ₂ _ x) wk*) })) ⟩
    _ ≡⟨ sym (Composeˢᴿ e _ wk Σ₂ wk*) ⟩
    _ ∎ }

Composeˢˢ : ∀ (e : Expr Γ₁ T) (σ₁ : n₁ →ˢ n₂) (σ₂ : n₂ →ˢ n₃) (Σ₁ : σ₁ ∣ Γ₁ ⇒ˢ Γ₂) (Σ₂ : σ₂ ∣ Γ₂ ⇒ˢ Γ₃) →
  σ₂ ∣ (σ₁ ∣ e ⋯ˢ Σ₁) ⋯ˢ Σ₂ ≡ (σ₁ ⨟ σ₂) ∣ e ⋯ˢ (σ₁ , σ₂ ∣ Σ₁ ⨾ Σ₂)
Composeˢˢ (` x)     _  _  _  _   = refl
Composeˢˢ (λx e)    σ₁ σ₂ Σ₁ Σ₂  = cong λx_ (trans (Composeˢˢ e _ _ _ _) (cong ((σ₁ ⨟ σ₂) ∣ e ⋯ˢ_) (Lift-Dist-Compˢˢ Σ₁ Σ₂)))
Composeˢˢ (Λα e)    σ₁ σ₂ Σ₁ Σ₂  = cong Λα_ (trans (Composeˢˢ e _ _ _ _) (cong (((σ₁ ⨟ σ₂) ↑ˢ) ∣ e ⋯ˢ_) (lift*-dist-Compˢˢ σ₁ σ₂ Σ₁ Σ₂)))
Composeˢˢ (e₁ · e₂) _  _  _  _   = cong₂ _·_ (Composeˢˢ e₁ _ _ _ _) (Composeˢˢ e₂ _ _ _ _)
Composeˢˢ (e ·* T′) σ₁ σ₂ Σ₁ Σ₂  = cong (_·* (T′ ⋯ˢ (σ₁ ⨟ σ₂))) (Composeˢˢ e _ _ _ _)