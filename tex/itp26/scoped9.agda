...
postulate
  ...
  lift-beta-suc-compˢˢ    : 
    ∀ {σ₁ :  S₁ ⊩[ q ] S₂} → {σ₂ : (s ∷ S₂) ⊩[ r ] S₃} →
    suc x ⋯ ((σ₁ ↑ˢ s) ;ˢˢ σ₂) ≡ x ⋯ (σ₁ ;ˢˢ (wk s ;ˢˢ σ₂))    -- rvarlift2ss
  comp-assocˢˢˢ           : 
    ∀ {σ₁ :  S₁ ⊩[ q ] S₂} → {σ₂ : S₂ ⊩[ r ] S₃} → {σ₃ : S₃ ⊩[ u ] S₄} →
    (σ₁ ;ˢˢ σ₂) ;ˢˢ σ₃  ≡ σ₁ ;ˢˢ (σ₂ ;ˢˢ σ₃)                     -- assenvsss
  distributivityˢˢ        : (t ∙ˢ σ₁) ;ˢˢ σ₂  ≡ ((t ⋯ σ₂) ∙ˢ (σ₁ ;ˢˢ σ₂))               -- mapenvss
...
_∶_→ˢ_ {S₁} {S₂} σ Γ₁ Γ₂ = ∀ (s : Sort) (x : S₁ ∋ s) (t : S₁ ∶⊢ s) → (Γ₁ ∋ x ∶ t) → Γ₂ ⊢ (x ⋯ σ) ∶ (t ⋯ σ) 
...
⊢↑ˢ : σ ∶ Γ₁ →ˢ Γ₂ → (t : S ∶⊢ s) → (σ ↑ˢ s) ∶ t ∷ₜ Γ₁ →ˢ ((t ⋯ σ) ∷ₜ Γ₂)
⊢↑ˢ ⊢σ _ _ (zero) _ refl = ⊢` {!   !} -- ⊢` ?
⊢↑ˢ {σ = σ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ⊢σ t _ (suc x) _ refl = {!   !} -- ⊢wkˢ Γ₂ (x ⋯ σ) (wk-drop-∈ x (Γ₁ _ x) ⋯ σ) (t ⋯ σ) (⊢σ _ x _ refl)
...
⊢[] : ∀ {Γ : Ctx S} {e : S ⊢ s} {t : S ∶⊢ s} → Γ ⊢ e ∶ t → (e ∙ˢ {!   !}) ∶ (t ∷ₜ Γ) →ˢ Γ
⊢[] ⊢t _ zero     _ refl = {!   !} -- ⊢t
