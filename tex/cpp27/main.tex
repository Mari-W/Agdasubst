...
%   trs/TRS.md     the rule-by-rule provenance table (NOT shipped)
%   supplement/    the Agda, the generator, the POPLmark development
...
\begin{abstract}
      ...
      Opaque definitions satisfy both.

      %%CHANGED (abstract): was the inductive-variables/surjective-pairing
      %%sentence -- dropped as too technical; first-class renamings added,
      %%and the close is now positive ("the rule set suffices throughout").
      We identify which version of the calculus can be oriented at all, and
      derive the rule set that follows. Renamings stay first-class in it, since
      a proof assistant needs them to define substitution by structural
      recursion and some proofs reason about them directly. A generator emits the whole development from a
      syntax description. Agda certifies local confluence and AProVE
      termination. Solving the POPLmark Challenge and POPLmark Reloaded on top
      exercises the result. The rule set suffices throughout, and a single law
      resists our orientation.
\end{abstract}
...
programming language metatheory. Compared to pen-and-paper proofs,
binders in proof assistants are a notorious source of uninteresting
technicalities and boilerplate. Many representations have been proposed to
%%CHANGED (intro para 1): three missing citations added (locally nameless,
%%HOAS, de Bruijn), and de Bruijn credited with alpha-equivalence.
contain them, among them locally nameless
binders~\cite{Chargueraud2012}, nominal syntax~\cite{10.1007/11532231_4,
Nominal2-AFP}, higher-order abstract syntax~\cite{PfenningElliott1988}, and
de~Bruijn indices~\cite{deBruijn1972}. De~Bruijn syntax makes $α$-equivalent
terms syntactically identical, so $α$-equivalence never has to be reasoned about,
and the syntax itself is easy to define. A development built on it still pays for
a long list of lemmas about substitution composition and about the interaction of
substitution with binders.

\emph{The $σ$-calculus and \textsc{Autosubst}.} On paper, substitution usually replaces a single variable. In a proof assistant
one instead works with parallel substitutions, which map variables to terms and
replace all of them simultaneously. The $σ$-calculus~\cite{10.1145/96709.96712} makes
such a map a first-class object with primitive syntactic operations for identity,
weakening, extension, composition and lifting, together with a rewrite system
%%CHANGED (intro): the de Bruijn algebra is now described for what it is,
%%and cited to Schaefer-Smolka-Tebbi CPP'15 instead of the Autosubst paper.
that normalizes expressions built from them. The de~Bruijn
algebra~\cite{10.1145/2676724.2693163} interprets those operations over de~Bruijn
terms, with substitutions modeled as functions from variables to terms. In that
model the rules are theorems rather than postulates, which is what makes them
usable for equational reasoning. \textsc{Autosubst}~\cite{schafer2015autosubst} and its successor
...
\emph{Rewrite rules in proof assistants.} The equalities so obtained are propositional. Normalization is a step the user
...
%%CHANGED (intro): the orphaned "a syntactic condition comes with it" is
%%folded in, and the condition is stated rather than named.
is confluent and terminating~\cite{10.1145/3434341}. Agda additionally applies a
rule by higher-order pattern unification~\cite{10.1007/978-3-642-21691-6_5}
against its left-hand side, so that side must not reduce and must be headed by a
defined symbol.

%%CHANGED (intro): heading renamed; "no left-hand side is ever stuck"
%%removed as too strong; the dangling "they" named.
\emph{Modeling the primitives.} That requirement is about how the primitives are
defined rather than about how substitutions are represented. Where the primitives
are defined by recursion, as functions of a variable or as vectors of terms, they
compute and the laws are provable. A definition that computes can also reduce
inside a left-hand side. Where the primitives are constructors instead, as when
the calculus's own syntax is the representation, every left-hand side is stuck
%%CHANGED (intro): Wadler rewritten -- minimal version of the route,
%%weakening only, and it costs pushing weakening inwards.  Our own
%%suc-vs-wk tension is now acknowledged, with the rule that settles it.
and nothing is derivable. Wadler takes a minimal version of
that route, making weakening a constructor and nothing else, which already
suffices to avoid renamings~\cite{Wadler_2024}. A constructor has nothing to
compute, so weakening does not push inwards. A weakened application stays
weakened, and every law that moves weakening through a term is propositional. The
same tension is present in our setting, between the variable constructor
$\bsym{\mathsf{suc}}$ and the map $\bsym{\mathsf{wk}}$, and the rule
$x\,[\,\bsym{\mathsf{wk}}\,] = \bsym{\mathsf{suc}}\,x$ settles it. That rule is
available to us because both sides are definitions, and \cref{sec:pre-sig} shows
what it costs elsewhere. Agda's opaque
definitions~\cite{gratzer2022controlling} give both properties at once.
...
%%CHANGED: missing space in the run-in heading.
\emph{Rewritable versions of the $σ$-calculus.} Not every version of the $σ$-calculus is
...
syntax in a proof assistant has native inductive variables instead. Having them
forces $x\,[\,\bsym{\mathsf{wk}}\,] = \bsym{\mathsf{suc}}\,x$, which identifies
the two spellings of a variable. That identification destroys the left-hand side
of $η$, which mentions its substitution twice and is therefore not
left-linear.
...
\begin{itemize}
      \item \textbf{A rewrite system.} The $σ$-calculus with first-class
            ...
            consistency is preserved. The rule set is derived, as a small base closed under three
            completion operators. Each operator carries a side condition saying
            when an image is unnecessary, so the absences are predicted along
            with the rules.
...
we register a different rule set. We do answer the corresponding question for a
concrete two-world system in \cref{sec:ext}.
...
postulates~\cite{schafer2015autosubst}. Variables are natural numbers, terms are
...
This section builds the rule set and says what fixes it. We give the primitives
...
metatheory (\cref{sec:mis-ex-proof}), and why the primitives must not unfold, together with
what the alternative representation would cost (\cref{sec:mis-ex}). Throughout, the multi-sorted scoped System~F of
...
and define instantiation with renamings first.
The equational theory then has to cover renamings as well as substitutions, and
to relate the two.
...
\begin{figure*}[t]
      ...
      \begin{minipage}[t]{0.50\linewidth}
            ...
            \ESubComp{}
      \end{minipage}
      \caption{The primitives for renaming (left) and for substitution (right).
...
constants \AgdaFunction{idˢ} and \AgdaFunction{wkˢ} are embedded renamings,
obtained through the coercion \AgdaFunction{⟨\_⟩}, and are \AgdaKeyword{INLINE}d
so that they do not reduce on a left-hand side.
...
Seventy-two rules is too many to read one at a time. Reading them that way would
also hide what matters about the number, which is that it is determined. The system is a small base closed under three completion operators,
and every rule belongs to exactly one group. \Cref{fig:mis-sig} shows the
...
below are the price of design decisions, and each comes with a side condition saying when an image is
...
the definitions of lifting, which we keep primitive. The last, $\mathsf{lift\text{-}id}$, is subsumed. Its left-hand side is a strict
instance of $\langle\cdot\rangle\text{-}\mathsf{lift}$'s, which already sends it to
...
\AgdaBound{S} a type one sort up, so \AgdaBound{Γ} \AgdaFunction{⊢} \AgdaBound{e}
\AgdaFunction{∶} \AgdaBound{t} reads as usual. \Cref{fig:refl} collects
everything the argument below depends on.
...
\emph{weakened}, since the body is typed in a longer context.
...
$e_1\,[\,e_2\,]_0$. Each of the three puts a substitution somewhere a proof will
later have to move it, and each therefore creates a demand for one law.
...
\AgdaFunction{]ˢ} in \AgdaBound{Γ₂}. The type is substituted along with the term,
which is what makes the notion compose. \AgdaFunction{sub-pres} then says that
applying such a substitution to a well typed term preserves typing, and
\AgdaFunction{sr} is subject reduction. Both are the expected inductions and use
two helpers, shown middle right. \AgdaFunction{⊢↑ˢ} lifts a well typed
substitution under a binder, and is proved from the corresponding statement for
renamings, which is what keeps the recursion structural.
\AgdaFunction{⊢[]} turns a well typed term into the well typed single
substitution that the two $β$-cases need.

Middle left are the three laws the three obligations create a demand for. A
de~Bruijn development states and applies each of them by hand. Here each holds by
\AgdaInductiveConstructor{refl}, because each is a normalization rather than a
lemma: the rules turn both sides into the same normal form, and
\AgdaInductiveConstructor{refl} sees that they agree. Written out for
$\mathsf{subst\text{-}commute}$, the classic substitution lemma, the two branches
are
{\small\begin{align*}
       & (t\,[\,{\Uparrow}σ\,])\,[\,t'[σ]\,]_0
...
The bottom of the figure closes the loop, and each clause can now be read against
the rule above it. In the \AgdaInductiveConstructor{⊢λ} case the induction
hypothesis types the body at $(\mathsf{weaken}\ t')\,[\,\Uparrow σ\,]$, since
that is where \AgdaInductiveConstructor{⊢λ} put it, while
\AgdaInductiveConstructor{⊢λ} demands $\mathsf{weaken}\,(t'\,[\,σ\,])$ to
rebuild the abstraction. That is $\mathsf{wk\text{-}comm}$. In the
\AgdaInductiveConstructor{⊢•} case the goal is
$(t'\,[\,t\,]_0)\,[\,σ\,]$ and the three sub-derivations supply
$(t'\,[\,\Uparrow σ\,])\,[\,t\,[\,σ\,]\,]_0$, which is
...
Every other clause of either proof needs nothing, and for two different reasons.
\AgdaInductiveConstructor{⊢Λ}, \AgdaInductiveConstructor{⊢·} and
\AgdaInductiveConstructor{⊢*} conclude at a type the substitution never has to be
moved through, so no equation arises. The $β$-case for
...
function extensionality. We keep functions as the more intuitive model. The laws
are theorems either way, so we can report that the $η$-laws are never once
...
buys the $η$-laws definitionally, from Agda's $η$-rule for records, and forfeits
the rules, because the operations then reduce on their own and every rule
mentioning one overlaps its defining clauses unjoinably. The trade is made per
symbol. Transparency buys $η$ and forbids rules. Opacity buys rules and forbids
$η$. We take opacity, and drop $η$ for the independent reason of
\cref{sec:pre-sig}.
...
\cref{sec:pre-agd} half discharged, is a claim about that language. This section
turns it into a claim about the method. \Cref{sec:gen} generates the
...
\cref{tab:groups}, only traversal depends on the signature, and it depends on it
in the simplest possible way, two rules per constructor. Everything else is
schematic in the constructor list, so the whole construction can be generated. We
provide a script that turns a higher-order abstract syntax (HOAS) description
into Agda code with the rules ready for rewriting. \Cref{fig:rel-hoa} is the
description of our running example, and it is the entire input: eleven
constructors across three sorts, with binders written as function arguments.
\textsc{Autosubst 2} does the same for Rocq, emitting the scoped syntax together
with the substitution lemmas that need induction over it.
...
can be encoded with additional sorts at the cost of standard library support. Two features are missing. A distinction between sorts with and without binders
would take an additional index on \AgdaDatatype{Sort}. Variadic syntax could be
encoded through arguments to the sort
...
boilerplate Agda code so that others can run it standalone. Beside the five
languages of \cref{sec:eval} we generate a dozen files from the HOAS
...
To test the construction at a scale where substitution reasoning hurts, we solved
...
published solutions. The claim rests on three measurements, each a property of the whole development
rather than of a chosen example.
...
measures the Challenge needs still take their own induction, since with an
abstract argument the traversal rules do not fire. Second, a rewrite system
constrains what may be matched as well as what may be proved. An earlier version
of our transitivity proof for subtyping recursed on
$Q\,[\,ξ\,]^{\mathsf{R}}$ to make the induction structural in $Q$. It type checks
in the ordinary sense and is then rejected by
...
The system does \emph{not} pass Agda's global \texttt{--confluence-check}, and the reason
matters, because an artifact reviewer will flip the pragma. Turning
...
hell, and it is a long-standing issue~\cite{stronglytyped}. Intrinsically typed System~F has been formalized in Agda before, executably and
...
to a scoped, multi-sorted setting~\cite{saffrich:LIPIcs.ITP.2024.32}. We believe generic
programming and rewriting the $σ$-calculus are orthogonal. Applying our rule set
to the universe of syntaxes with binding would remove the need for external code
...
on generic theorems about substitution. Its experimental support for rewriting
with $σ$-calculus laws misses rules and does not pass the confluence check.
...
%%CHANGED (conclusion): trimmed by ~4 lines to bring the body back to 12
%%pages after the 3.4 rewrite grew.
We have turned the normalization procedure of the $σ$-calculus into definitional
equalities in Agda. Two contrary things are wanted at once. The
primitives must compute, so that the theory can be instantiated with proofs, and
stay symbolic, so that the rules are eligible for rewriting. Opaque definitions
resolve it.

The theory we register is $σ_\Uparrow$ instantiated once per world, with the rules
that first-class renamings force between the two. Which laws survive is forced,
because native inductive variables and surjective pairing cannot coexist in one
rewrite system. Agda certifies local confluence and AProVE termination, a
generator emits the system from a syntax description, and the POPLmark
challenges exercise it at scale. One law resists orientation, and we say why.
