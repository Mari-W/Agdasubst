...
-- `SystemF-canonicity.agda` truncated after the last layer-(i) REWRITE
-- pragma, with the prototype's §0 layer-0 pragma LEFT IN.
-- Measured: 102 non-joinable pairs, ALL attributed to the §0
-- EXPRESSION-TRAVERSAL rules (Compositionality*/Beta-*); 0 to layer (i).
-- The prototype's own §0 comment states those rules cannot be certified,
-- and that the module "carries no --local-confluence-check" — which
-- contradicts its own OPTIONS line.  See §2 of REPORT-canonicity-port.md.
-- Nothing imports this file.
-- ⚠ DECISIVE MEASUREMENT for REPORT-canonicity-port.md, Task 1 ⚠
-- This is `SystemF-canonicity.agda` TRUNCATED immediately after the last
-- layer-(i) REWRITE pragma.  Its only purpose is to answer, without
-- inference, whether the layer-(i) environment σ-calculus
--     ∷-⟨⟩↑  ∷-⟨⟩wk  ⟨⟩-id  ⟦⟧-ren  ⟪⟫-⟨⟩  ∷-⟪⟫↑  ⟪⟫-∙  ⟦⟧-sub
-- passes --local-confluence-check on top of the CURRENT esop27/SystemF.
-- If this file exits 0, the answer is yes and the count is 0.
...
-- ════════════════════════════════════════════════════════════════════
module SystemF-canonicity-core where
...
sn-λx : ∀ {n} {Γ : Ctx n} {T₁ T₂} {e : Expr (Γ ▷ T₁) T₂} → SN (λx e)
sn-λx = acc λ ()
