...
lift*-dist-compˢˢ b (suc n) = cong (_↑ˢ b) (lift*-dist-compˢˢ b n)

⟨⟩-lift* : ∀ b n → (⟨ ξ ⟩ ↑ˢ*[ b ] n) ≡ ⟨ ξ ↑ᴿ*[ b ] n ⟩
⟨⟩-lift* b zero    = refl
⟨⟩-lift* b (suc n) = cong (_↑ˢ b) (⟨⟩-lift* b n)

-- The completion companions of the iterated layer.  The base rule set
-- subsumes every SINGLE-lifting companion, so each case here is a bare
-- `cong` on the corresponding base star law.

lift*-dist-compᴿᴿ-var : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {s} {x : (ext* b n S₁) ∋ s}
  {ξ₁ : S₁ →ᴿ S₂} {ξ₂ : S₂ →ᴿ S₃} →
  (x [ (ξ₁ ↑ᴿ*[ b ] n) ]ᴿ) [ (ξ₂ ↑ᴿ*[ b ] n) ]ᴿ ≡ x [ ((ξ₁ ⨟ᴿ ξ₂) ↑ᴿ*[ b ] n) ]ᴿ
lift*-dist-compᴿᴿ-var b n {x = x} = cong (λ z → x [ z ]ᴿ) (lift*-dist-compᴿᴿ b n)

lift*-dist-compᴿᴿ-⨟ᴿ : ∀ {S₁ S₂ S₃ S₄} (b : Sort) (n : ℕ) {ξ₁ : S₁ →ᴿ S₂} {ξ₂ : S₂ →ᴿ S₃}
  {ξ′ : (ext* b n S₃) →ᴿ S₄} →
  (ξ₁ ↑ᴿ*[ b ] n) ⨟ᴿ ((ξ₂ ↑ᴿ*[ b ] n) ⨟ᴿ ξ′) ≡ ((ξ₁ ⨟ᴿ ξ₂) ↑ᴿ*[ b ] n) ⨟ᴿ ξ′
lift*-dist-compᴿᴿ-⨟ᴿ b n {ξ′ = ξ′} = cong (_⨟ᴿ ξ′) (lift*-dist-compᴿᴿ b n)

lift*-dist-compˢˢ-⨟ : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {S₄} {σ₁ : S₁ →ˢ S₂} {σ₂ : S₂ →ˢ S₃}
  {τ : (ext* b n S₃) →ˢ S₄} →
  (σ₁ ↑ˢ*[ b ] n) ⨟ ((σ₂ ↑ˢ*[ b ] n) ⨟ τ) ≡ ((σ₁ ⨟ σ₂) ↑ˢ*[ b ] n) ⨟ τ
lift*-dist-compˢˢ-⨟ b n {τ = τ} = cong (_⨟ τ) (lift*-dist-compˢˢ b n)

lift*-dist-compᴿˢ-⨟ : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {S₄} {ξ : S₁ →ᴿ S₂} {σ : S₂ →ˢ S₃}
  {τ : (ext* b n S₃) →ˢ S₄} →
  ⟨ ξ ↑ᴿ*[ b ] n ⟩ ⨟ ((σ ↑ˢ*[ b ] n) ⨟ τ) ≡ ((⟨ ξ ⟩ ⨟ σ) ↑ˢ*[ b ] n) ⨟ τ
lift*-dist-compᴿˢ-⨟ b n {τ = τ} = cong (_⨟ τ) (lift*-dist-compᴿˢ b n)

lift*-dist-compˢᴿ-⨟ : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {S₄} {σ : S₁ →ˢ S₂} {ξ : S₂ →ᴿ S₃}
  {τ : (ext* b n S₃) →ˢ S₄} →
  (σ ↑ˢ*[ b ] n) ⨟ (⟨ ξ ↑ᴿ*[ b ] n ⟩ ⨟ τ) ≡ ((σ ⨟ ⟨ ξ ⟩) ↑ˢ*[ b ] n) ⨟ τ
lift*-dist-compˢᴿ-⨟ b n {τ = τ} = cong (_⨟ τ) (lift*-dist-compˢᴿ b n)

lift*-dist-compᴿˢ-var : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {s} {x : (ext* b n S₁) ∋ s}
  {ξ : S₁ →ᴿ S₂} {σ : S₂ →ˢ S₃} →
  (x [ (ξ ↑ᴿ*[ b ] n) ]ᴿ) [ (σ ↑ˢ*[ b ] n) ]ˢ ≡ x [ ((⟨ ξ ⟩ ⨟ σ) ↑ˢ*[ b ] n) ]ˢ
lift*-dist-compᴿˢ-var b n {x = x} = cong (λ z → x [ z ]ˢ) (lift*-dist-compᴿˢ b n)

⟨⟩-comp-⨟-lift*-dist-compᴿᴿ : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {S₄} {ξ₁ : S₁ →ᴿ S₂} {ξ₂ : S₂ →ᴿ S₃}
  {τ : (ext* b n S₃) →ˢ S₄} →
  ⟨ ξ₁ ↑ᴿ*[ b ] n ⟩ ⨟ (⟨ ξ₂ ↑ᴿ*[ b ] n ⟩ ⨟ τ) ≡ ⟨ (ξ₁ ⨟ᴿ ξ₂) ↑ᴿ*[ b ] n ⟩ ⨟ τ
⟨⟩-comp-⨟-lift*-dist-compᴿᴿ b n {τ = τ} = cong (λ z → ⟨ z ⟩ ⨟ τ) (lift*-dist-compᴿᴿ b n)
...
lift*-dist-compᴿˢ-⨟-var b n {x = x} {τ = τ} =
  cong (λ z → x [ (z ⨟ τ) ]ˢ) (lift*-dist-compᴿˢ b n)

⟨⟩-split*-tail : ∀ {S₁ S₂ S₃} (b : Sort) (n : ℕ) {S₄} {σ : S₁ →ˢ S₂} {ξ : S₂ →ᴿ S₃}
  {ξ′ : (ext* b n S₃) →ᴿ S₄} →
  (σ ↑ˢ*[ b ] n) ⨟ ⟨ (ξ ↑ᴿ*[ b ] n) ⨟ᴿ ξ′ ⟩ ≡ ((σ ⨟ ⟨ ξ ⟩) ↑ˢ*[ b ] n) ⨟ ⟨ ξ′ ⟩
⟨⟩-split*-tail b n {ξ′ = ξ′} = cong (_⨟ ⟨ ξ′ ⟩) (lift*-dist-compˢᴿ b n)
