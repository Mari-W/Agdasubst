...
-- type equality+
opaque 
  _$_  : Type ∅ (J ⇒ K) → Type ∅ J → Type ∅ K
  (` ()) $ t′
  (λα t) $ t′ = t [ t′ ]* 

  -- hold by sigma calc
  traverseᴿ-$ : (T₁ $ T₂) ⋯ᴿ ρ ≡ (T₁ ⋯ᴿ ρ) $ (T₂ ⋯ᴿ ρ)
  traverseᴿ-$ {T₁ = λα T₁} = refl
  traversal-$ : (T₁ $ T₂) ⋯ˢ σ  ≡ (T₁ ⋯ˢ σ) $ (T₂ ⋯ˢ σ)
  traversal-$ {T₁ = λα T₁} = refl
...
progress (λx_ e) NV  = done (v-λx e)
progress (Λα_ e) NV with progress e NV 
... | step x = step (ξ-Λ x)
... | done v = done (v-Λα v)
progress (e₁ · e₂) NV with progress e₁ NV 
... | step x         = step (ξ-· x)
... | done (v-λx e)  = step β-λ 
progress (e • T′) NV with progress e NV 
... | step x         = step (ξ-• x)
... | done (v-Λα e)  = step β-Λ 
