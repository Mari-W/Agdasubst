...
\authornote{
      This is a copy submitted as a Master’s thesis in Computer Science at the University of Freiburg.\\The first examiner is Prof. Dr. Peter Thiemann, and the second examiner is Prof. Dr. Andreas Podelski.\\My matriculation number is 4933008. The thesis was written between 2025-04-09 and 2025-09-08.\\\\
...
\begin{abstract}
      ...
      which, for instance, establish the behavior of composition and moving
      ...
      substitutions down to a syntactic level. The $σ$-calculus provides a set of
      syntactic reduction rules for terms and substitutions ensuring confluence and
      strong normalization. This calculus allows equations between terms to be solved
      by comparing their normal forms. By applying the reduction rules until a normal
      form is reached, we obtain an automatic procedure that eliminates the need for
      manually applying substitution lemmas.
      ...
      This embedding is challenging because it requires precise control over the
      reduction behavior of Agda definitions, substitutions must be defined
      structurally in terms of renamings, and multiple variable sorts must be
      ...
      separately, and then present a uniform treatment that allows results about both
      to be proven once, avoiding duplication. In both cases, thanks to the
      $σ$-calculus laws embedded as rewrite rules, equations are resolved
      % : is that true? are they definitional? (Cockx expresses that more carefully)
      automatically by reduction, yielding definitional equalities in cases where
      lemmas would otherwise need to be applied.
\end{abstract}
...
the \AgdaField{default} field into scope with the \AgdaKeyword{open} keyword
...
\noindent Here, Agda infers from the type \AgdaDatatype{ℕ} of \AgdaNumber{0} that the
...
\AgdaKeyword{REWRITE} pragma: \ERewriteIt{}From this point on, Agda treats
\AgdaBound{n} \AgdaFunction{+} \AgdaNumber{0} as definitionally equal to
\AgdaBound{n}: \ERewriteEx{}To maintain consistency, Agda requires that all
...
called \AgdaBound{Scope} and ranged over by \AgdaBound{S}, together with a
target sort. A type \AgdaBound{S} \AgdaDatatype{⊢} \AgdaBound{s} describes a
term of sort \AgdaBound{s} with free variables whose sorts are listed in
\AgdaBound{S}. Variable positions are represented by \AgdaBound{S}
\AgdaDatatype{∋} \AgdaBound{s}, a proof that a variable of sort \AgdaBound{s}
appears in \AgdaBound{S}. The proof acts as the de Bruijn index. Using the
...
\begin{itemize}
      \item \AgdaField{K-id/‘} converts a variable into a veriable or term \AgdaBound{S} \AgdaBound{∋/⊢ᴷ} \AgdaBound{s}.
            For the Varfiable Kit, \AgdaBound{\_∋/⊢ᴷ\_} is instantiated to \AgdaDatatype{\_∋\_}, so the operation needs to act as the identity.
...
each field a corresponding version without the \AgdaBound{K}-prefix. These are
...
implicitly parameterized over a Kit \AgdaBound{K} with Mode \AgdaBound{M} and
can be uniformly stated. All the primitves are marked \AgdaKeyword{opaque}.
...
generalized weakening operator \AgdaFunction{\_;wk}, which can later be
rewritten as an actual composition with the weakening primitive. Similarly,
once term traversal is available, the \AgdaFunction{id/‘} and
...
\AgdaBound{K₁}-Map with a \AgdaBound{K₂}-Map, yielding a \AgdaBound{K₁⊔K₂}-Map:
\AComposeKit{}The operation \AgdaField{C-\_\&/⋯\_} unifies lookup and
traversal. It takes a variable or term \AgdaBound{x/t} (according to
\AgdaBound{K₁}) and a renaming or substitution \AgdaBound{ϕ} (according to
\AgdaBound{K₂}), and applies \AgdaBound{ϕ} to \AgdaBound{x/t}, yielding a
variable or term (according to \AgdaBound{K₁⊔K₂}). From unified
...
\AgdaDatatype{Lock} is private, it can only be constructed within the library.
Any attempt to declare an instance outside the library would fail, because the
required field \AgdaField{K-lock} of type \AgdaDatatype{Lock} cannot be filled.
...
whether a given \AgdaBound{K} is \AgdaFunction{V} or \AgdaFunction{T}, based on
its Mode.
...
the main difficulty is to compute \AgdaBound{C₃} and \AgdaBound{C₄}.

We can try to define a function \ACompCKitSafe{} for this purpose, that
evaluates to \AgdaFunction{Cᴿ}, if \AgdaBound{K₁} has Mode
\AgdaInductiveConstructor{Vᴹ} and to \AgdaFunction{Cˢ}, if \AgdaBound{K₁} has
Mode \AgdaInductiveConstructor{Tᴹ}. The function can be readily defined, but
...
inspection we confirm that \AgdaFunction{Cᴿ} and \AgdaFunction{Cˢ} are the only
...
constructor in the \AgdaDatatype{Mode} type, fails because the
\AgdaFunction{image} can no longer be computed for a Kit whose mode index
...
using the rewrite rules \ACKitRenRed{} for \AgdaFunction{Cᴿ} and \ACKitSubRed{}
for \AgdaFunction{Cˢ}. The unsafe function is only available from within the
...
operator, we can connect $K_1$ \AgdaFunction{⊔} $ (K_2 $ \AgdaFunction{⊔}
$K_4)$ with \AgdaBound{K₅} in the outer Compose Kit on the right side of the
...
user, because it is declared \AgdaKeyword{private}.
...
\AgdaFunction{V} or \AgdaFunction{T}, we use the variables $ρ$ and $σ$
...
operator\footnote{We considered keeping the specialized operators, but this
      ...
      contributes to progress.}. The laws \AgdaFunction{wk}, \AgdaFunction{ext₀}, and
...
our suspicion when examining the \AgdaFunction{comp} and
...
directly after applying \AgdaFunction{comp}, by specializing on $\varphi_1$ or
...
can be swapped with the identity substitution: \AgdaFunction{swap} :
\ANormId{}. If the \AgdaFunction{swap} law were added to the equational theory,
...
\AgdaFunction{associativity}, \AgdaFunction{distributivity}, and
\AgdaFunction{interact} laws.
...
duality between the general definition of composition \AgdaFunction{comp} and
...
and typing relation is expressed using the operator \AgdaFunction{\_∶⊢\_}. This
operator computes the sort of the third index of the relations based on the
sort of the second. With the \AgdaFunction{\_∶⊢\_} operator, we can define a
...
that lifiting of a substitution preserves typing (\AgdaFunction{⊢↑ˢ}), show
that general substitution preserves typing (\AgdaFunction{\_⊢⋯ˢ\_}) as well.
...
\AgdaFunction{id↑≡id}\;:\;\AIdLift{}, and for compositionality under a binder, we
rely on \AgdaFunction{dist–↑–;}\;:\;\ADistLift{}. Both of these are already
...
the case of \AgdaFunction{β-λ}\footnote{See the proof provided by
...
reproving syntax-specific laws. The \AgdaInductiveConstructor{`X} constructor
allows the addition of \AgdaBound{S′} many binders. In the current version,
\textsc{Agdasubst} does not support variadic (unknown amount of newly bound
variables) binders. The rewrite rules only apply correctly when \AgdaBound{S′}
