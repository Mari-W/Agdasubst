...
\emph{Defined or declared.} The syntactic condition is a condition on
...
laws are provable, and no left-hand side is ever stuck. Where they are
constructors, as when the calculus's own syntax serves as the representation,
every left-hand side is stuck and nothing is derivable. This is the route Wadler
takes, and it avoids renamings altogether~\cite{Wadler_2024}. It costs the rule
...
that pushes weakening through a term stays propositional. Agda's
...
under \texttt{--safe}, the four $η$-laws included, each of the four by case
analysis on the map. The function model derives them from function
extensionality, which we postulate. Since the laws are theorems under either
representation, we can report that the $η$-laws are never once applied by hand
across the POPLmark development.
...
between the two. Which laws survive is forced. Native inductive
variables and surjective pairing cannot coexist in one rewrite system, and that
single fact fixes most of the design. Agda certifies the resulting system locally confluent, and AProVE proves it
confluent and terminating. A generator emits it from a syntax description, and
the POPLmark Challenge and POPLmark Reloaded exercise it at scale. One law
resists our orientation, and we have said precisely why.
