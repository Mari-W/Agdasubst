...
functions and thereby makes the rules provable rather than
...
We drop $η$ too, for the independent reason of \cref{sec:pre-sig}, so on that
axis the two designs agree. They differ on what can be proved. In our development
all 72 registered rules and the nine unregistered ones are theorems, discharged
against the function model, and that is what makes the exclusions measurable. Because we can state and prove the
...
\Cref{sec:introduction} discussed what that representation costs. He also uses
vectors for substitutions, and so cannot instantiate the full rule set with
proofs.
