...
postulate
  ...
  -- fun-ext λ _ → : ∀{ℓ₁ ℓ₂} → ExtensionalityImplicit  ℓ₁ ℓ₂
...
(ρ₁ ;ᴿˢ σ₂) _ x =  (x &ᴿ ρ₁) &ˢ σ₂ 
{-# INLINE _;ᴿˢ_ #-}

_;ˢᴿ_ : (S₁ →ˢ S₂) → (S₂ →ᴿ S₃) → (S₁ →ˢ S₃)
(σ₁ ;ˢᴿ ρ₂) _ x = (x &ˢ σ₁) ⋯ᴿ ρ₂
{-# INLINE _;ˢᴿ_ #-}
-- opaque end  
...
-- 
    ...
    η-lawᴿ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))
    η-lawˢ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))

    distributivityᴿᴿ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))
    distributivityᴿˢ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))
    distributivityˢᴿ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))
    distributivityˢˢ = fun-ext λ _ → (fun-ext (λ { zero → refl ; (suc x) → refl }))

    comp-idₗᴿᴿ = refl
    comp-idᵣᴿᴿ = refl
    comp-idₗᴿˢ = refl
    comp-idₗˢˢ = refl
    comp-idᵣˢᴿ {σ = σ} = fun-ext λ _ → (fun-ext λ x → right-idᴿ (x &ˢ σ))
    comp-idᵣˢˢ {σ = σ} = fun-ext λ _ → (fun-ext λ x → right-idˢ (x &ˢ σ))
    ...
    compositionalityˢᴿ {σ₁ = σ₁} {ρ₂ = ρ₂} (λx e)       = cong λx_ (trans (compositionalityˢᴿ {σ₁ = (σ₁ ↑ˢ _)} {ρ₂ = (ρ₂ ↑ᴿ _)} e) (cong (_⋯ˢ_ {S₂ = expr ∷ _} e) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿᴿ _) (sym (compositionalityᴿᴿ _)) }))))
    compositionalityˢᴿ {σ₁ = σ₁} {ρ₂ = ρ₂} (Λα e)       = cong Λα_ (trans (compositionalityˢᴿ {σ₁ = (σ₁ ↑ˢ _)} {ρ₂ = (ρ₂ ↑ᴿ _)} e) (cong (_⋯ˢ_ {S₂ = type ∷ _} e) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿᴿ _) (sym (compositionalityᴿᴿ _)) }))))
    compositionalityˢᴿ {σ₁ = σ₁} {ρ₂ = ρ₂} (∀[α∶ k ] t) = cong₂ ∀[α∶_]_ (compositionalityˢᴿ k) (trans (compositionalityˢᴿ {σ₁ = (σ₁ ↑ˢ _)} {ρ₂ = (ρ₂ ↑ᴿ _)} t) (cong (_⋯ˢ_ {S₂ = type ∷ _} t) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿᴿ _) (sym (compositionalityᴿᴿ _)) })))) 
    ...
    compositionalityˢˢ {σ₁ = σ₁} {σ₂ = σ₂} (λx e)       = cong λx_ (trans (compositionalityˢˢ {σ₁ = (σ₁ ↑ˢ _)} {σ₂ = (σ₂ ↑ˢ _)} e) (cong (_⋯ˢ_ {S₂ = expr ∷ _} e) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿˢ (σ₁ _ x)) (sym (compositionalityˢᴿ (σ₁ _ x))) }))))
    compositionalityˢˢ {σ₁ = σ₁} {σ₂ = σ₂} (Λα e)       = cong Λα_ (trans (compositionalityˢˢ {σ₁ = (σ₁ ↑ˢ _)} {σ₂ = (σ₂ ↑ˢ _)} e) (cong (_⋯ˢ_ {S₂ = type ∷ _} e) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿˢ (σ₁ _ x)) (sym (compositionalityˢᴿ (σ₁ _ x))) }))))
    compositionalityˢˢ {σ₁ = σ₁} {σ₂ = σ₂} (∀[α∶ k ] t) = cong₂ ∀[α∶_]_ (compositionalityˢˢ k) (trans (compositionalityˢˢ {σ₁ = (σ₁ ↑ˢ _)} {σ₂ = (σ₂ ↑ˢ _)} t) (cong (_⋯ˢ_ {S₂ = type ∷ _} t) (fun-ext λ _ → (fun-ext λ { zero → refl; (suc x) → trans (compositionalityᴿˢ (σ₁ _ x)) (sym (compositionalityˢᴿ (σ₁ _ x))) }))))
    ...
    associativityˢᴿᴿ {σ₁ = σ₁} = fun-ext λ _ → (fun-ext λ x → compositionalityᴿᴿ (x &ˢ σ₁))
    associativityˢᴿˢ {σ₁ = σ₁} = fun-ext λ _ → (fun-ext λ x → compositionalityᴿˢ (x &ˢ σ₁))
    associativityˢˢᴿ {σ₁ = σ₁} = fun-ext λ _ → (fun-ext λ x → compositionalityˢᴿ (x &ˢ σ₁))
    associativityˢˢˢ {σ₁ = σ₁} = fun-ext λ _ → (fun-ext λ x → compositionalityˢˢ (x &ˢ σ₁))
    ...
    coincidence-foldᵀ {t = t} {t′ = t′} {ρ = ρ} rewrite swap-id ρ = 
      (t ⋯ˢ ((t′ ⋯ᴿ ρ) ∙ˢ (λ _ x → ` ρ _ x))) ≡⟨ cong (_⋯ˢ_ t) (sym (distributivityˢᴿ {σ₁ = idˢ} {ρ₂ = ρ})) ⟩ 
