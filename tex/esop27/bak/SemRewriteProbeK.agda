...
-- This file is EXPECTED TO FAIL.  `agda SemRewriteProbeK.agda` → exit 42.
-- Round fallback: the control file plus the SINGLE highest-payoff semantic rule `⟦⟧ᵀ-wk` (mirror of `interact`).
-- Measured: 4 non-joinable pairs vs compositionalityᴿᴿ, compositionalityˢᴿ, beta-fold-ˢᴿ and `_[_]ᴿ`-clause3 — so even the minimal payoff-bearing rule is not confluent alone.
-- Nothing imports this file.  It exists so that §7 of
-- REPORT-adequacy.md is reproducible.
-- FALLBACK ATTEMPT: the control file plus the SINGLE highest-payoff
-- semantic rule, `⟦⟧ᵀ-wk` (mirror of strat's `interact`).
-- Measured: 4 non-joinable pairs — vs `compositionalityᴿᴿ`,
-- `compositionalityˢᴿ`, `beta-fold-ˢᴿ`, and `_[_]ᴿ`-clause3 (the ∀
-- traversal clause).  So even the minimal payoff-bearing rule is not
-- confluent on its own: its LHS `⟦ T [ wkᴿ ]ᴿ ⟧ᵀ (A ∷ η)` carries the
-- COMPUTED argument `T [ wkᴿ ]ᴿ`, which overlaps strat's composition and
-- traversal rules.  Evidence for §7.6 of REPORT-adequacy.md.
-- ════════════════════════════════════════════════════════════════════
-- THE `rw` VARIANT: SystemF-adequacy.agda plus the largest SEMANTIC
-- rewrite rule set that could be certified locally confluent.
--
-- That set is the singleton { ⟦⟧ᵀ-wk }, the semantic mirror of strat's
-- `interact`:      ⟦ weaken T ⟧ᵀ (A ∷ η)  ↦  ⟦ T ⟧ᵀ η
-- See REPORT-adequacy.md §7 for the nine rounds that established which
-- further semantic laws can and cannot join it.  Everything else in this
-- file is identical to the control; the diff IS the measurement.
--
...
--
-- Module-for-module correspondence and the transport comparison are in
-- esop27/REPORT-adequacy.md.
--
...
--
-- POSTULATES: none beyond `fun-ext` (inherited from SystemF-strat).
-- HOLES: none.
-- ════════════════════════════════════════════════════════════════════
module SemRewriteProbeK where
...
-- `Evaluation.agda` + `BigStep.agda`, 53 + 44 lines there.
...
-- Their StratF/TypeSubstPropertiesSem.agda (146 lines): `Ren*`,
...
-- ★ THE ONE CERTIFIED SEMANTIC REWRITE RULE ★
-- the semantic mirror of strat's `interact`.  Registered here; §7 of the
-- report records that no other semantic law could be added without
-- breaking --local-confluence-check.
⟦⟧ᵀ-wk : ∀ {Δ l l′} (T : Type Δ l′) (A : Set l) (η : Env* Δ) →
         ⟦ weaken T ⟧ᵀ (A ∷ η) ≡ ⟦ T ⟧ᵀ η
⟦⟧ᵀ-wk T A η = ⟦⟧ᵀ-ren wkᴿ η (A ∷ η) (Ren*ᵀ-wk η A) T
...
-- difference; it is scored separately in the report.
...
-- ★ rw: the `subst id (sym (⟦⟧ᵀ-ren …))` the control needs here is GONE;
-- `⟦ weaken T ⟧ᵀ (A ∷ η)` and `⟦ T ⟧ᵀ η` are now definitionally equal.
lookupᵥ (suc* {T = T} x)  (A ∷ η) γ = lookupᵥ x η γ
...
-- Their StratF/LRVren.agda (2013 lines) and StratF/LRVsub.agda (2296).
-- Their headline statements carry transports:
--
...
--! LRVren   (their LRVren-eq / LRVren-eq′ — note: NO subst₂, no transport)
...
--! LRVsub   (their LRVsub — note: NO subst, no `congωl`, no inlined
...
--! MCGlookup
-- their `𝓖-lookup` (LRVren.agda:1850–1931).  The `suc*` case is the one
