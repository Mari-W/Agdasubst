...
-- (⊢-subTy from FOpH1.Typing); β and type-β are handled by those two lemmas.
module FOpH1.SR where
...
⊢lam-inv (⊢lam d) = d
-- bodyOf-∀↑ is refl once B's thinning head is exposed ⇒ split B, NO subst
⊢Lam-inv : ∀ {Γ : Cx Θ}{t C} → Γ ⊢ Lam t ∶ C → wkCx Γ ⊢ t ∶ bodyOf C
⊢Lam-inv (⊢Lam {B = _ ⇑ os _} d) = d
⊢Lam-inv (⊢Lam {B = _ ⇑ o' _} d) = d
