...
data Expr {S : Scope} (Γ : Ctx S) : S ⊢ type → Set where
  ...
  λx_   : Expr (Γ ⸴ t₁) t₂ → 
          Expr Γ (t₁ ⇒ t₂) 
  Λα_   : {t : (type ∷ S) ⊢ type} → Expr (Γ ⸴★) t → 
          Expr Γ (∀[α∶ k ] t)
  _·_   : Expr Γ (t₁ ⇒ t₂) → Expr Γ t₁ → 
          Expr Γ t₂
  _∙_   : {t : (type ∷ S) ⊢ type} → 
          Expr Γ (∀[α∶ k ] t) → (t' : S ⊢ type) → 
          Expr Γ (t [ t' ])
--! }
...
--! SubComp {
_;ˢˢᴱ_ : (Γ₁ →ˢᴱ[ σ₁ ] Γ₂) → (Γ₂ →ˢᴱ[ σ₂ ] Γ₃) → 
  (Γ₁ →ˢᴱ[ σ₁ ;ˢˢ σ₂ ] Γ₃)
(σᴱ₁ ;ˢˢᴱ σᴱ₂) _ x = (σᴱ₁ _ x) ⋯ˢᴱ[ _ ] σᴱ₂
--! }
...
η-idᴿᴱ : {Γ : Ctx S} → _∙ᴿᴱ_ {t = t} {ρ = idᴿ} {Γ₁ = Γ} zero (_;ᴿᴿᴱ_ {ρ₁ = idᴿ} {ρ₂ = idᴿ} wkᴱ idᴿᴱ) ≡ idᴿᴱ 
η-idᴿᴱ = fun-ext λ _ → fun-ext λ { zero → refl ; (suc x) → refl }

η-idˢᴱ : {Γ : Ctx S} → _∙ˢᴱ_ {t = t} {σ = idˢ} {Γ₁ = Γ} (` zero) (_;ᴿˢᴱ_ {ρ = idᴿ} {σ = idˢ} wkᴱ idˢᴱ) ≡ idˢᴱ 
η-idˢᴱ = fun-ext λ _ → fun-ext λ { zero → refl ; (suc x) → refl }

η-idᴿᴱ★ : {Γ : Ctx S} → _∙ᴿᴱ★_ {Γ₁ = Γ} {ρ = wk} zero (_;ᴿᴿᴱ_ {ρ₁ = wk} {ρ₂ = idᴿ} wkᴱ★ idᴿᴱ) ≡ idᴿᴱ 
η-idᴿᴱ★ = fun-ext λ _ → fun-ext λ { (suc★ x) → refl }

η-idˢᴱ★ : {Γ : Ctx S} → _∙ˢᴱ★_ {Γ₁ = Γ} {σ = wk ;ᴿˢ idˢ}  (` zero) (_;ᴿˢᴱ_ {ρ = wk} {σ = idˢ} wkᴱ★ idˢᴱ) ≡ idˢᴱ
η-idˢᴱ★ = fun-ext λ _ → fun-ext λ { (suc★ x) → refl }

right-idᴿᴱ : (e : Expr Γ t) → e ⋯ᴿᴱ[ idᴿ ] idᴿᴱ ≡ e
right-idᴿᴱ (` x)       = refl
right-idᴿᴱ (λx e)      = cong λx_ (trans (cong (e ⋯ᴿᴱ[ idᴿ ]_) η-idᴿᴱ) (right-idᴿᴱ e)) 
right-idᴿᴱ (Λα e)      = cong Λα_ (trans (cong ((e ⋯ᴿᴱ[ idᴿ ↑ᴿ _ ]_)) η-idᴿᴱ★) (right-idᴿᴱ e))
right-idᴿᴱ (e₁ · e₂)   = cong₂ _·_ (right-idᴿᴱ e₁) (right-idᴿᴱ e₂)
right-idᴿᴱ (e ∙ t′)    = cong (_∙ t′) (right-idᴿᴱ e)

right-idˢᴱ : (e : Expr Γ t) → e ⋯ˢᴱ[ idˢ ] idˢᴱ ≡ e
right-idˢᴱ (` x)       = refl
right-idˢᴱ (λx e)      = cong λx_ (trans (cong (e ⋯ˢᴱ[ idˢ ]_) η-idˢᴱ) (right-idˢᴱ e)) 
right-idˢᴱ (Λα e)      = cong {!  Λα_  !} {!   !}
right-idˢᴱ (e₁ · e₂)   = cong₂ _·_ (right-idˢᴱ e₁) (right-idˢᴱ e₂)
right-idˢᴱ (e ∙ t′)    = {! cong (_∙ t′)  ?  !}  -- cong (_∙ t′) (right-idˢᴱ e)