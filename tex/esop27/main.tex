...
% @MW: explain confluence + inst for rule set = sound and consistent 
...
\begin{figure}[t]
  ...
  \begin{tabular}{|l|rr|rr|}
    \hline
                                          & \multicolumn{2}{c|}{\textbf{Rewriting}}
  ...
  \end{tabular}
  \caption{%
    Code transitively required to prove the four expression-level
    compositionality lemmas in
    two Agda formalizations of System~F. LOC counts non-blank,
    non-comment lines; type-check times are wall-clock, cold cache.
  }
  \label{fig:compositionality-cost}
\end{figure}
...
Here we only state the \emph{type} of the proof and the induction case for application, 
which is among the short ones.
% \EFFusionSubSub
% The mapping from their to our syntax (in order) is
% \begin{itemize}
%   \item $\Delta$ $\mapsto$ $n$
%   \item \AgdaFunction{ESub} $\mapsto$ \AgdaFunction{$\_∣\_⋯ˢ\_$}
%   \item $\sigma^*$ $\mapsto$ $\eta$
%   \item \AgdaFunction{fusion-Tsub-Tsub} $\mapsto$ \AgdaFunction{compose$^{SS}$}
%   \item \AgdaFunction{$\_∘^{SS}$} $\mapsto$ \AgdaFunction{$\_⨟\_$}
%   \item \AgdaFunction{$\_$>>$^{SS}\_$} $\mapsto$ \AgdaFunction{$\_∣\_⨟\_$}
% \end{itemize}
The authors use heterogeneous equality to proof the lemma, 
thereby avoiding some of the troubles arising from transfer hell.
% \LambdaEx
The full proof can be found in Appendix \ref{appendix:appendix-proof}
