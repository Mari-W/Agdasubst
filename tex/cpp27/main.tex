...
\begin{abstract}
      ...
      %this is not clear, and too technical i think? what is inductive variables and surjective paring, this is too technbical?
      %also shouldnt we mention that we want first class renamings (one of our biggest burdons, which are required in proof assitants (and in some proofs))
      We identify which variant of the calculus
      ...
      %i dont like this sentence ("it")
      A generator emits it from a syntax description. 
      
      Agda certifies
      local confluence and AProVE termination. Solving the POPLmark Challenge
      and POPLmark Reloaded on top exercises the result. 
      
      %the laws we cannot register seems bad, rephrase positelvy, the rule set suffices or something, but juist a single law resists blabla
      The laws we cannot
      register are never needed, and only a single law resists our orientation.
\end{abstract}
...
contain them, among them locally nameless binders%cite needed
, nominal
syntax~\cite{10.1007/11532231_4, Nominal2-AFP}, higher-order abstract syntax%cite needed
,
and de~Bruijn indices%originbal de bruijn paper cite need
. 
%mention they solve alpha equivalence for free somehow
De~Bruijn syntax is canonical and easy
...
that normalizes expressions built from them. 
% is that true? check again, is it really functions and does the algebra model something?
The de~Bruijn algebra, which models
...
packages substitutions over several sorts into vectors, 
and promotes renamings,
...
is confluent and terminating~\cite{10.1145/3434341}. 
%dont like the flow here this next sentence stands alone and only makes sense with what follows
A syntactic condition comes
...
%dont like the title, its not specific and uses non standard vocab
\emph{Defined or declared.} 
%its not called the "syntactic condition", describe it, it says that the LHS cannot reduce and must be a defined (function, in this case) symbol 
The syntactic condition is a condition on
definitions rather than on representations. Where the primitives are defined by
recursion, as functions of a variable or as vectors of terms, they compute, the
laws are provable, 
% bold claim, removwe
and no left-hand side is ever stuck. 

%what is they
Where they are
constructors, as when the calculus's own syntax serves as the representation,
every left-hand side is stuck and nothing is derivable. 
%he doesnt fully take that route, only work weakening, whcih suffices to skip renamigns, so this is an mibnimal instace of it
This is the route Wadler
takes, and it avoids renamings altogether~\cite{Wadler_2024}. 
% not true, read it, he doesnt have suc, to do suc he has wk 0 etc, so this is exactly what he spares, the rpoblem is he can have wk in the syntax also on ohter terms and they dont compute to the inside
%ALSO but we exactly have the issue of suc vs wk?? 
It costs the rule
$x\,[\,\bsym{\mathsf{wk}}\,] = \bsym{\mathsf{suc}}\,x$, which is then no longer a
theorem, so a variable acquires two inequivalent representations and every law
that pushes weakening through a term stays propositional. 
