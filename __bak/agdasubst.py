...
TOKEN_REGEX: list[tuple[TokenType | None, re.Pattern]] = [
...
def consume(tokens: list[Token], pos: int, expected_type: TokenType | None = None) -> tuple[Token, int]:
...
def parse_signature(tokens: list[Token]) -> Signature:
    ...
    while peek(tokens, pos).type != TokenType.EOF:
        ...
        if peek(tokens, pos).type == TokenType.TYPE_KW:
            ...
            if not parts:
                raise SyntaxError(f"Constructor {id_token.value} has no type signature.")
            
            target_sort = parts[-1].target_type
            if parts[-1].is_binder:
                raise SyntaxError(f"Constructor {id_token.value} cannot return a function/binder. Found: {parts[-1]}")
            
            arguments = parts[:-1]
            sig.constructors.append(ConstructorDecl(
                name=id_token.value,
                arguments=arguments,
...
def parse_argument(tokens: list[Token], pos: int) -> tuple[Argument, int]:
    if peek(tokens, pos).type == TokenType.LPAREN:
        ...
        raise SyntaxError(f"Unexpected token in type signature: {peek(tokens, pos).value} at line {peek(tokens, pos).line}")
...
def generate_map_clauses(sig: Signature, op_symbol: str, map_var: str, lift_op: str, indent: str = "") -> str:
    # Generalized function for Renaming and Substitution
    clauses: list[str] = []
...
def generate_variables(sig: Signature) -> str:
    # Re-scan to find all used variable names (e.g., ty0, tm1)
...
def generate_id_lemma(sig: Signature, lemma_name: str, op_symbol: str, lift_lemma: str) -> str:
    # Generalized function for Right Identity lemmas (renaming and substitution)
    # op_symbol expects '⋯ˢ_' or '⋯ᴿ_'
    clauses: list[str] = []
    max_len = 0
    data: list[tuple[str, str]] = []
    ...
    for c in sig.constructors:
        ...
        for arg in c.arguments:
            ...
            if arg.is_binder:
                binders = " ∷ ".join(reversed(arg.binder_types))
                explicit_list = f"({binders} ∷ [])"
                # proof = f"(trans (cong1 ({var_name} ⋯ˢ_) (beta-lift-idˢ* {explicit_list})) (right-idˢ {var_name}))"
        ...
        args_pattern = " ".join(arg_vars)
        lhs = f"{lemma_name} ({c.name} {args_pattern})" if args_pattern else f"{lemma_name} {c.name}"
...
def generate_compositionality_lemma(sig: Signature, lemma_name: str, map_op: str, comp_lemma: str) -> str:
    # Generalized function for Compositionality lemmas (RR, RS, SR, SS)
    # map_op expects '⋯ᴿ_' or '⋯ˢ_'
    clauses: list[str] = []
    max_len = 0
    data: list[tuple[str, str]] = []
    ...
    for c in sig.constructors:
        ...
        for arg in c.arguments:
            ...
            if arg.is_binder:
                binders = " ∷ ".join(reversed(arg.binder_types))
                explicit_list = f"({binders} ∷ [])"
                # proof = f"(trans (compositionalityᴿᴿ {var_name}) (cong1 ({var_name} ⋯ᴿ_) (lift-dist-comp*ᴿᴿ {explicit_list})))"
...
def generate_rewrite_block(sig: Signature) -> str:
    ...
    # We use the names from the existing templates (beta-lift-id etc)
    # matching the user's requested structure but with correct names.
    return f"""
{{-# REWRITE
...
def render_template(congs: str, sorts: str, constructors: str, renaming: str, 
                  variables: str, substitution: str, traversal: str, 
                  right_ids: str, right_id: str, compositionality_rr: str,
                  compositionality_rs: str, compositionality_sr: str, 
                  compositionality_ss: str, rewrite_block: str) -> str:
    
    # Static parts defined as simple strings to avoid f-string escaping hell with Agda's braces
    
    part1 = """{-# OPTIONS --rewriting --local-confluence-check --double-check #-}
...
def generate_agda(sig: Signature) -> str:
    ...
    return render_template(
        ...
        right_ids=generate_id_lemma(sig, "right-idˢ", "⋯ˢ_", "beta-lift-idˢ*"),
...
def main():
    parser = argparse.ArgumentParser(description="Generate Agda boilerplate from a signature file.")
    ...
    try:
        ...
        agda_code = generate_agda(signature)
