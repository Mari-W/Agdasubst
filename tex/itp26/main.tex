...
normalization procedure using the $σ$-calculus rules for unscoped syntax
definitions inside the Rocq proof assistant. To normalize terms with parallel
substitutions one needs to apply a tactic inside a proof goal.
...
Most importantly, \textsc{Autosubst 2}, in constrast to the first version,
...
rewrite rule can be applied to a stuck term. To do so, Agda requires that the
left-hand side is a stuck term on a defined symbol. To model parallel
substitution in proof assistants, we have essentially two choices: data types
or functions. When we model substitution as functions, some primitive
operations of the $σ$-calculus generate stuck lambdas, thereby violating the
syntactic restriction that the left-hand side must be a stuck symbol and not a
lambda.

Others tried to model substitution with constructors, but then we miss the
...
unfold their definitions to prove the rules using their underlying model.
...
exemplified by System~F. We discuss future and related work in \cref{sec:rwk}
...
non-unfolding symbols. This feature is usually used to improve type checking
...
declare some definitions opaque, such that the blocked symbols qualify for
rewriting.
...
\AgdaBound{s} and a \AgdaDatatype{Scope} \AgdaBound{S}. Modes distinguish
constructors for variables and terms. Variables are represented across multiple
sorts simultaneously by natural numbers of type \AgdaBound{S} \AgdaDatatype{∋}
\AgdaBound{s} and act as de~Bruijn indices. The position of \AgdaBound{s} in
\AgdaBound{S} directly corresponds to the de~Bruijn index of the variable.
Using the syntax unified over sorts, we can later define renamings and
substitutions that operate uniformly across all sorts, thereby avoiding vector
substitutions. Unifying over modes, allows us to state some $σ$-calculus laws
...
well-founded recursion). Because of that we follow
Adams~\cite{10.1007/11617990_1} to resolve the issue by first defining
...
any multi-sorted and scoped syntax as well as simple intrinsically typed syntax
...
forward function composition. Lifting \AgdaFunction{\_↑ᴿ\_} is defined directly
to avoid a definition for the extension of renamings, which is unnecessary
because renamings merely need to shift variables. The traversal operation
...
across both modes to match the definition of substitution instantiation later.
The computational behavior of renamings follows the behavior of functions
almost entirely. We require only a few additional \AgdaKeyword{REWRITE} rules
later to extend the definitional equalities of renamings.
...
operator we define the identity substitution \AgdaFunction{idˢ} and weakening
substitution \AgdaFunction{wkˢ} directly from renamings. To prevent Agda from
rejecting rewrite rules due to reductions on the left-hand side, we
\AgdaKeyword{INLINE} these symbols.
...
satisfying the requirements of structural recursion. Instantiation with a
...
their reduction allows us to rewrite the $σ$-calculus rules on the blocking
symbols. For example, the current definition for composition without blocked
reduction would reduce to a lambda function, which would render most of the
\textbf{Interaction rules} not admissable for rewriting. The definition of
instantiation over both modes, allows us to do lookup of a variable in a
substitution also by usage of the blocking symbol for instantiation, instead of
function application. We will state the equations we wish to rewrite later only
in terms of blocking symbols, without using lambdas or function application. We
go into more detail in \Cref{sec:mis-ex}.
...
\Cref{fig:mis-sig} presents the $σ$-calculus rules with first-class renamings
...
that the lifting the identity renaming remains the identity. This property does
not hold definitionally because lifting of renamings is defined via a case
split on the incoming variable. However, to preserve confluence with respect to
the Monad rules, this equality is required to hold definitionally. All other
properties we expect of renamings such as associativity hold definitionally
already.
...
in terms of composition with the weakening substitution, which is not possible
to define directly, due to the aforementioned termination issues. This change
us to draw on the complete theory of the $σ$-calculus, when solely reasoning
about substitutions. We can normalize every expression, only involving
substitutions and no renamings, into a normal form suitable for syntactic
comparison.
...
$η$-law for renamings where instantiation appears as a blocking symbol.

The \textbf{Coincidence rules} emerge from the interaction between renamings
and substitutions~\cite{10.1145/3293880.3294101}. They allow us to recover
renamings from their embedding as substitutions. We state them uniformly for
variables and terms \AgdaBound{x/t} instantiated with substitutions, because we
also want to coerce variable lookup in substitutions to variable lookup in
renamings.

The extraction of renamings from deeply nested substitutions in general
requires a dedicated solving strategy~\cite{10.1145/3293880.3294101}. In
addition to the obvious \AgdaFunction{coincidence} rule, we require a specific
coincidence variant called \AgdaFunction{coincidence-fold} for the proof of
subject reduction of System~F. Every proof might require more coincidence
lemmas and we are currently trying to figure out the patterns. For instance,
proving subject reduction might require the inclusion of lemmas in the style of
\AgdaFunction{coincidence-fold} with $n$ extensions where $n$ is the maximum
arity of binders in the object language. The inclusion of more coincidence
lemmas usually does not affect confluence.
...
rewrite rules where instantiation of variables is the head symbol.
...
the subsequent code. We briefly demonstrates this with a proof of subject reduction
...
\AgdaInductiveConstructor{\_•\_} which is normally required in standard proofs.
...
Blocking the reduction behavior of substitution primitives allowed us to
successfully rewrite all $σ$-calculus rules. We now explain why this approach
works, and more specifically why other attempts did not work.

The primary issue is that composition unfolds to a lambda abstraction. If we
...
violating the constraint that rewrite rules must have blocking symbols on the
...
re-establishes the computational behavior of composition, but leads to problems
with higher-order unification. To our suprise, Agda accepts this rule together
with the interaction rules, such as \AgdaFunction{comp-idₗ}, but it fails to
perform higher-order pattern unification correctly afterwards. More
specifically, it does not apply the $η$-laws anymore, even though manual
application using propositional equality reasoning works. We argue Agda should
probably reject such a construction.
...
substitutions using lambdas, we then obtain fewer definitional equalities. As a
consequence, we recommend using only instantiation for variable lookup and only
the two already defined composition operators.
...
vector-structured data type. This data type would at least need to include an
...
blocking symbol suitable for rewriting. However with extension as constructor
the $η$-law is not instantiatable with a proof, because this would require that
the extension constructor has computational behavior.

Removing the $η$-law from the equational theory preserves confluence, but
misses important definitional equalities. Although preventing some definitional
equalities, the vector representation could maybe be useful for an attempt at
...
chains. In this work we do both: Using the semantic model to prove the laws but
also treating the symbols syntactically to predicably apply rewrite rules.

\section{Future and Related Work}\label{sec:fwk}
...
To be precise, they generate the scoped syntax data types and all substitution
lemmas that require induction over the syntax for the Rocq proof assistant.
...
Our code generation script\footnote{We used GenAI in the process of writing
      this script.} can be found in the supplement, but represents merely a proof of
concept for a standalone library. The script includes a parser for HOAS
descriptions and ad-hoc code generation. It is written in Python with strict
type-checking enabled and includes all the boilerplate Agda code, so it can
serve as a single-file standalone tool for others to try. We include a dozen
Agda files which we generated from HOAS descriptions found in the examples
directory of the \textsc{Autosubst 2} git repository.

\subsection{Implications for Intriniscally Typed Syntax}
Complex interinsically typed syntax, where substitution already appears as an
operation on the type index of the syntax, complicates proofs significantly
because of so-called `transfer hell' (or `transport hell'). Because
substitutions without rewriting enjoy few definitional equalities, we need to
apply general properties of the transfer lemma (\AgdaFunction{subst} in Agda)
every time we manipulate proofs over expressions that involve type
substitution.

The `transfer hell' issue for intrinsically typed syntax is long standing
issue~\cite{article}. An elaborate example close to our work in Agda is given
by the construction of a binary logical relation for intrinsically typed
System~F~\cite{10.1145/3678000.3678201}, where the authors spend thousands of
lines reasoning about uninteresting manipulation of transfers. More
specifically, while reasoning over type substitutions is still easy in this
scenario, proving lemmas over type-in-expression substitution is complicated
immensely, because type substitution also affects the index of the expression
that is substituted in. After rewriting the $σ$-calculus rules for type
substitutions, their problems in the proofs for expression substitutions should
effectively be gone completely.
...
\subsection{Mechanizing Substitutions}\label{sec:rwk}
...
rewrite rules, but lacks the ability to instatitate the rules with proofs to
...
rules and higher-order unification.

Successfully rewriting the $σ$-calculus rules safely also allows to pursue
interesting new ventures: More complex intrinsically typed representations for
syntax that do not end up in transfer hell, the question of whether this
