...
postulate
  ...
  distributivityᴿᴿ        : (x ∙ᴿ ρ₁) ;ᴿᴿ ρ₂  ≡ ((x ⋯ˢ ρ₂) ∙ᴿ (ρ₁ ;ᴿᴿ ρ₂))               -- mapenvss
  interactᴿ               : wk s ;ᴿᴿ (x ∙ᴿ ρ) ≡ ρ                                       -- shiftconss
...
weaken {s′ = s} t = t ⋯ˢ wk {q = V} _ -- t ⋯ˢ wk _
...
--! TypingR
data _⊢[_]_∶_ : Ctx S → Mode → S ⊢[ q ] s → S ∶⊢ s → Set
...
--!! WTS
_∶_→ˢ_ : S₁ ⊩[ q ] S₂ → Ctx S₁ → Ctx S₂ → Set
_∶_→ˢ_ {S₁} {S₂} σ Γ₁ Γ₂ = 
  ∀ (s : Sort) (x : S₁ ∋ s) (t : S₁ ∶⊢ s) → (Γ₁ ∋ x ∶ t) → Γ₂ ⊢ tm⊑ ⊑t (x ⋯ˢ σ) ∶ (t ⋯ˢ σ) 
...
⊢wk : ∀ (Γ : Ctx S) (x : S ∋ s) t (t′ : S ∶⊢ s′) → Γ ∋ x ∶ t → (t′ ∷ₜ Γ) ∋ (suc x) ∶ (weaken t) 
⊢wk _ _ _ _ (⊢x refl) = (⊢x refl)

⊢↑ᴿ : ρ ∶ Γ₁ →ᴿ Γ₂ → (t : S₁ ∶⊢ s) → (ρ ↑ᴿ s) ∶ (t ∷ₜ Γ₁) →ᴿ ((t ⋯ˢ ρ) ∷ₜ Γ₂)
⊢↑ᴿ ⊢ρ _ _ (zero) _ (⊢x refl) = (⊢x refl) -- refl
⊢↑ᴿ {ρ = ρ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ⊢ρ t _ (suc x) _ (⊢x refl) = {!  ⊢wk Γ₂ (x ⋯ˢ ρ) (wk-drop-∈ x (Γ₁ _ x) ⋯ˢ ρ) (t ⋯ˢ ρ) (⊢ρ _ x _ (⊢x refl))  !}
--
--! RPT
-- _⊢⋯ˢ_ : ∀ {e : S₁ ⊢ s} {t : S₁ ∶⊢ s} →
--   ρ ∶ Γ₁ →ᴿ Γ₂ →
...
⊢↑ˢ ⊢σ _ _ (zero) _ (⊢x refl) = ⊢` (⊢x refl) -- ⊢` ?
⊢↑ˢ {σ = σ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ⊢σ t _ (suc x) _ _ = {!   !}
