...
\begin{figure*}[!t]
      ...
      \ECaseBeta{}
      \caption{Left, the three substitution lemmas the System~F metatheory needs,
            each holding by \AgdaInductiveConstructor{refl}. Right, the two
            statements that use them. Below, the only three clauses of either
            proof in which a law is spent, with the law named. Every other clause
            needs none.}\label{fig:refl}
\end{figure*}
...
call-by-value small-step semantics \AgdaDatatype{\_↪\_}, both in the supplement.
A context \AgdaDatatype{Ctx} \AgdaBound{S} assigns to every variable in scope
\AgdaBound{S} a type one sort up, so \AgdaBound{Γ} \AgdaFunction{⊢} \AgdaBound{e}
\AgdaFunction{∶} \AgdaBound{t} reads as usual.

\Cref{fig:refl} (right) shows the two statements. A substitution is well typed,
written \AgdaBound{σ} \AgdaFunction{∶} \AgdaBound{Γ₁} \AgdaFunction{→ˢ}
\AgdaBound{Γ₂}, when it sends every variable \AgdaBound{x} typed
\AgdaBound{t} in \AgdaBound{Γ₁} to a term \AgdaBound{x} \AgdaFunction{[}
\AgdaBound{σ} \AgdaFunction{]ˢ} typed \AgdaBound{t} \AgdaFunction{[}
\AgdaBound{σ} \AgdaFunction{]ˢ} in \AgdaBound{Γ₂}. The type of a variable is
substituted along with the term, which is what makes the statement compose.
\AgdaFunction{sub-pres} then says that applying such a substitution to a well
typed term preserves typing, and \AgdaFunction{sr} is subject reduction. The
proofs are the expected inductions, with one helper: \AgdaFunction{⊢↑ˢ} lifts a
well typed substitution under a binder, and it is proved from the corresponding
statement for renamings, which is what keeps the recursion structural.

\Cref{fig:refl} (left) shows the three substitution lemmas the two proofs need.
A de~Bruijn development states and applies each of them by hand. Here each holds
by \AgdaInductiveConstructor{refl}, because each is a normalization rather than a
...
The bottom of the figure shows where each law is spent. In the \AgdaInductiveConstructor{⊢λ} case the induction hypothesis
types the body at $(\mathsf{weaken}\ t')\,[\,\Uparrow σ\,]$ while
\AgdaInductiveConstructor{⊢λ} demands $\mathsf{weaken}\,(t'\,[\,σ\,])$, which is
$\mathsf{wk\text{-}comm}$. In the \AgdaInductiveConstructor{⊢•} case the two
sides are $(t'\,[\,\Uparrow σ\,])\,[\,t\,[\,σ\,]\,]_0$ and
$(t'\,[\,t\,]_0)\,[\,σ\,]$, which is $\mathsf{subst\text{-}commute}$. In the
$β$-case for \AgdaInductiveConstructor{λx\_} the redex is typed at
$(\mathsf{weaken}\ t_2)\,[\,e_2\,]_0$ where the goal is $t_2$, which is
