...
declare some definitions opaque, such that the stuck symbols qualify for
rewriting.
...
resolve the issue by first defining instantiation for renamings. We then extend
the equational theory to handle first-class renamings and to connect
substitutions with renamings with the rules from Autosubst
2~\cite{10.1145/3293880.3294101}. We use the multi-sorted and scoped version of
System~F from \Cref{fig:pre-svm} as an example for any scoped syntax. The
approach described in this section directly translates to any multi-sorted and
scoped syntax as well as simple intrinsically typed syntax where the type index
of the syntax has type \AgdaDatatype{Set}.
...
their reduction allows us to rewrite the $σ$-calculus rules on the stuck
...
substitution also by usage of the stuck symbol for instantiation, instead of
function application. We will state the equations we wish to rewrite later only
in terms of stuck symbols, without using lambdas or function application. We go
into more detail in \Cref{sec:mis-ex}.
...
\Cref{fig:mis-sig} presents the $σ$-calculus rules with f
irst-class renamings
...
that lifting the identity renaming remains the identity. This property does not
hold definitionally because lifting of renamings is defined via a case split on
the incoming variable. However, to preserve confluence with respect to the
Monad rules, this equality is required to hold definitionally. All other
...
reasoning about substitutions. We can normalize every expression, only
involving substitutions and no renamings, into a normal form suitable for
...
$η$-law for renamings where instantiation appears as a stuck symbol.
...
lemmas. For instance, proving subject reduction requires the inclusion of
