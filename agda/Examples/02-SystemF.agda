...
_T↑ₖᵣ_ : Δ₁ T⇒ᵣ Δ₂ → (ℓ : Level) → (ℓ ∷ Δ₁) T⇒ᵣ (ℓ ∷ Δ₂)
ρ T↑ₖᵣ _ = (here refl) T∷ᵣ (ρ T；ᵣᵣ Twkᵣ)

Twk : Type Δ ℓ → Type (ℓ' ∷ Δ) ℓ
Twk T = T T⋯ᵣ Twkᵣ

_T↑ₖₛ_ : Δ₁ T⇒ₛ Δ₂ → (ℓ : Level) → (ℓ ∷ Δ₁) T⇒ₛ (ℓ ∷ Δ₂)
σ T↑ₖₛ _ = (` (here refl)) T∷ₛ (σ T；ₛᵣ Twkᵣ)
...
(Λα ℓ , e) ET⋯ᵣ ρ★ = Λα ℓ , (e ET⋯ᵣ (ρ★ T↑ₖᵣ ℓ))
...
(Λα ℓ , e) ET⋯ₛ σ★ = Λα ℓ , (e ET⋯ₛ (σ★ T↑ₖₛ ℓ))
