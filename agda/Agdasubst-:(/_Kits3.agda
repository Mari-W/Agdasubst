...
variable
  kt kt₁ kt₂ kt₃ kt₄ kt′ kt₁′ kt₂′ kt₃′ kt₄′ : KitTag  

module KitsWithSort (Sort : SORT) where
    open SortsWithSort Sort 
    open SortsMeta hiding (_∋/⊢_)
    ...
    record Syntax : Set₁ where
      ...
      record Kit (tag : KitTag) : Set₁ where
        constructor mkKit
        pattern
        _∋/⊢_ = idx tag
        field
      ...
      _∋/⊢[_]_ : Scope → Kit kt → BindSort → Set
      _∋/⊢[_]_ S K s = Kit._∋/⊢_ K S s
 
      _–[_]→_ : Scope → Kit kt → Scope → Set
      ...
      opaque
        ...
        -- REWRITE   
        &-def₁ : ⦃ K : Kit kt ⦄ (x/t : S₂ ∋/⊢ s) (ϕ : S₁ –[ K ]→ S₂) → zero & (x/t ∙ ϕ) ≡ x/t
        &-def₁ _ _ = refl

        &-def₂ : ⦃ K : Kit kt ⦄ (x/t : S₂ ∋/⊢ s) (ϕ : S₁ –[ K ]→ S₂) → (suc x′) & (x/t ∙ ϕ) ≡ x′ & ϕ  
        &-def₂ _ _  = refl

        id-def : ⦃ K : Kit kt ⦄ (x : S ∋ s) → x & (id ⦃ K ⦄) ≡ id/` x
        id-def _ = refl

        ∷-def₁ : ⦃ K : Kit kt ⦄ (x/t : S₂ ∋/⊢ s) (ϕ : S₁ –[ K ]→ S₂) → zero & (x/t ∙ ϕ) ≡ x/t
        ∷-def₁ _ _ = refl

        ∷-def₂ : ⦃ K : Kit kt ⦄ (x/t : S₂ ∋/⊢ s) (x' : S₁ ∋ s′) (ϕ : S₁ –[ K ]→ S₂) → suc x′ & (x/t ∙ ϕ) ≡ x′ & ϕ
        ∷-def₂ _ _ _ = refl

        wk-def : ⦃ K : Kit kt ⦄ (x : S ∋ s) → x & (wk {s = s′}) ≡ id/` (suc x)
        wk-def = wk-id/` _
      ...
      record Traversal : Set₁ where
        ...
        field  
          _⋯_    : ∀ ⦃ K : Kit kt ⦄ → S₁ ⊢ s → S₁ –[ K ]→ S₂ → S₂ ⊢ s
          ⋯-var  : ∀ ⦃ K : Kit kt ⦄ → (x : S₁ ∋ s) (ϕ : S₁ –[ K ]→ S₂) →
                     (` x) ⋯ ϕ ≡ `/id ⦃ K ⦄ (x & ϕ)
          ⋯-id   : ∀ ⦃ K : Kit kt ⦄ → (t : S ⊢ s) →
                     t ⋯ id ⦃ K ⦄ ≡ t
        instance
        ...
        postulate
          unique-Kᵣ-instance : (K : Kit Ren) → K ≡ Kᵣ
        ...
        postulate
          unique-Kₛ-instance : (K : Kit Sub) → K ≡ Kₛ
        ...
        _⨆_ : KitTag → KitTag → KitTag
        Ren ⨆ Ren = Ren
        _  ⨆ _ = Sub

        instance
          MAGIC : ∀ ⦃ K₁ : Kit kt₁ ⦄ ⦃ K₂ : Kit kt₂ ⦄ → Kit (kt₁ ⨆ kt₂) 
          MAGIC {Ren} {Ren} = Kᵣ
          MAGIC {Ren} {Sub} = Kₛ
          MAGIC {Sub} {Ren} = Kₛ
          MAGIC {Sub} {Sub} = Kₛ
        ...
        _⊔_ : {kt₁ kt₂ : KitTag} (K₁ : Kit kt₁) (K₂ : Kit kt₂) → Kit (kt₁ ⨆ kt₂) 
        _⊔_ {kt₁} {kt₂} K₁ K₂ = let instance _ = K₁; _ = K₂ in MAGIC 
        ...
        record WkKit (K : Kit kt): Set₁ where 
        ...
        wkKit : (K : Kit kt) → WkKit K
        wkKit {Ren} K rewrite unique-Kᵣ-instance K = Wᵣ 
        wkKit {Sub} K rewrite unique-Kₛ-instance K = Wₛ
       
        record ComposeKit (K₁ : Kit kt₁) (K₂ : Kit kt₂) : Set where
        ...
        record ComposeTraversal : Set₁ where
          field
            ⋯-fusion : ∀ ⦃ K₁ : Kit kt₁ ⦄ ⦃ K₂ : Kit kt₂ ⦄ ⦃ C : ComposeKit K₁ K₂ ⦄
