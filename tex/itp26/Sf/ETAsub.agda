...
-- A substitution = a thing-with-thinning in Δ for each variable of Γ, presented
-- RECURSIVELY as a tuple.  Sub Δ (tt∷Γ) is a product `Tm↑Δ × Sub Δ Γ`, i.e. an
-- η-RECORD: it enjoys DEFINITIONAL η (σ ≡ (head σ , tail σ)).  THAT is what makes
-- the surjective-pairing law SCons and IdCons hold by `refl` — an inductive `data`
-- has no η, so those two laws were stuck before.  `_,-_` and `[]` are pattern
-- synonyms over the constructor, so the rest of the development is unchanged.
Sub : Scope → Scope → Set
Sub Δ []       = ⊤
Sub Δ (tt ∷ Γ) = (Tm ↑ Δ) × Sub Δ Γ
pattern _,-_ σ u = (u , σ)
infixl 5 _,-_
εₛ : ∀ {Δ} → Sub Δ []          -- the empty substitution (was the `[]` constructor)
εₛ = tt
...
opaque
  ...
  wkSub {Γ = []}     σ              = σ
  wkSub {Γ = tt ∷ Γ} (σ ,- (t ⇑ θ)) = wkSub σ ,- (t ⇑ o' θ)
...
opaque
  ...
  sub var                 (σ ,- u) = u                          -- structural lookup, no σ x
...
opaque
  unfolding sub
  varlookup : ∀ {Δ}(u : Tm ↑ Δ) → sub var (εₛ {Δ} ,- u) ≡ u
  varlookup u = refl
