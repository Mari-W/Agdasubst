...
of proof assistants, many representations have been proposed, including locally
nameless binders, nominal syntax, higher-order abstract syntax, and de Bruijn
indices. However, choosing the right representation is difficult. While de
Bruijn syntax provides a promising representation, developments based on it
still include substantial proof overhead by requiring numerous lemmas to manage
substitution composition and interactions with binders.
...
symbol and not a lambda. Others tried to model substitution with constructors,
but then we lack the computational behavior of substitutions required to
instantiate the full rule set of the $σ$-calculus with proofs, and in turn lose
preservation of logical consistency. Modeling substitutions as vectors does not
constitute a complete semantic model of the $σ$-calculus.
...
exemplified by an instantiation for System~F. We discuss ongoing and future work in \cref{sec:fwk},
...
using propositional equality, which is a cumbersome process as Agda often lacks
...
the rewritten rules are supported by proofs, and we asume that the rewrite
system is terminating, local confluence is enough to preserve
...
sorts: expressions, types, and kinds~\footnote{Technically kinding for System~F
      is trivial and could be omitted.}. We use intrinsically scoped syntax, to
...
\AgdaDatatype{∋} \AgdaBound{s} and act as de~Bruijn indices. The length of
\AgdaBound{S} encodes the amount of variables in scope and the position of
...
$σ$-calculus laws parametric in the mode.
...
lifting operator which pushes substitutions under binders using shifting:
\begin{align*}
...
implicit, because Agda often fails to infer it, complicating proofs
...
standard forward function composition. Extension \AgdaFunction{\_∙ᴿ\_} inserts
the respective variable for de Bruijn index \AgdaInductiveConstructor{zero}
into the renaming. Lifting \AgdaFunction{\_↑ᴿ\_} is defined in terms of
extension with variable \AgdaInductiveConstructor{zero} and composition with
the weakening renaming. We do not mark lifting opaque, because it is merely a
alias for a term constructed from other primitives. The traversal operation
...
The definitions of instantiation over both modes, allows us to perform lookup
of a variables in substitutions (or renamings) also by using of the stuck
symbol for instantiation (\AgdaBound{x} \AgdaFunction{⋯ˢ} \AgdaBound{σ}),
instead of function application (\AgdaBound{σ} \AgdaBound{\_} \AgdaBound{x}).
Because we need to state the $σ$-calculus rules only in terms of stuck symbols,
without using lambdas or function application, we require an additional symbol
for variable lookup. We go into more detail in \Cref{sec:mis-ex}.
...
as Agda equalities admissible for rewriting. All rules from the second
paragraph of the \AgdaKeyword{REWRITE} block were omitted from the figure and can be found in the
supplement.

Agda accepts the rule set as locally confluent which is important for safety.
We are also able to prove all the rules before rewriting them, to guarantee
consistency. We now discuss the changes to the standard $σ$-calculus variant
from \Cref{sec:pre-sig}.
...
in terms of composition with the weakening substitution, which we were not
possible to do earlier, due to the aforementioned termination issues. This
change allows us to draw on the complete theory of the $σ$-calculus, when
solely reasoning about substitutions. To save space, we have omitted the
...
later. However, others proofs might require more coincidence lemmas. For
instance, proving subject reduction in general requires the inclusion of lemmas
in the style of \AgdaFunction{coincidence-fold} with $n$ extensions where $n$
is the maximum arity of binders in the object language. The inclusion of more
coincidence lemmas usually does not affect confluence.
...
could maybe still be useful if one attempts to formalize
...
de~Bruijn algebra provided a semantic model for the $σ$-calculus using
functions, enabling proofs for the $σ$-calculus rules and applicability for
automated propositional equality reasoning. In this work we do both: Using the
semantic model to prove the laws but also treating the symbols syntactically to
predicably apply rewrite rules.
...
easily generalize to other intrinsically typed formalizations and render
explicit transfers unnecessary.
...
also rewrite $σ$-calculus rules with support for first-class renamings in Rocq,
...
rewrite rules~\cite{leray_et_al:LIPIcs.ITP.2024.26}, and we expect our approach can be translated to Rocq. However,
Rocq lacks the ability to instantiate the rewrite rules with
proofs, and we would then lose
...
rules and higher-order pattern unification. \bibliography{references}
