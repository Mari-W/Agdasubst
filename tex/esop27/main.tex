...
such $n$  a \emph{scope}.
Variables for scope $n$ are drawn from the type $\AgdaDatatype{Fin}\ n$, i.e.,
...
The first group presents computation rules for substitutions starting
...
The second group contains laws about composition of substitutions.
They describe how composition interacts with the other substitution primitives.
...
The extraction of renamings from substitutions in general requires a dedicated solving
strategy~\cite{10.1145/3293880.3294101} and we have omitted some additionally required coincidence laws
from the figure.

Orienting these equations from left to right results in a confluent
rewrite system that we install in Agda using its
\AgdaKeyword{REWRITE} pragma \cite{10.1145/3434341}. From now on, all equations in
\cref{fig:system-f-sigma-calculus} are definitional!

As a sanity check, we show that lifting a substitution and the
action of a substitution satisfy the functor laws. All of these lemmas
hold definitionally, some correspond directly to equations from the $\sigma$-
...
system interact in non-trivial ways, so having the right notion of
when it is safe to add them is important. We follow~\cite{} to justify
making the equations from \cref{fig:system-f-sigma-calculus}
definitional and recall their reasoning for the safety of the proposed
Rewriting Type Theory (RTT), which has been successfully implemented
...
what have done in the previous section. The two properties we wish
to preserve when adding rewrite rules are \emph{soundness} (i.e.,
subject reduction) and \emph{consistency}. In particular, subject
reduction is an implicit assumption of the type checker that we 
need to take care to uphold.
...
The additionally rewritten rules must agree with all of them to be confluent. 
...
Some $\beta$ and $\iota$ reductions arising from our functional
definitions of renamings and substitutions in the previous section
interfere with the intended rewrite equalities. This is why we
explicitly hide these definitions inside an \AgdaKeyword{opaque} block,
so that only the equations we install with \AgdaKeyword{REWRITE} drive
their reduction behaviour.

Consistency is preserved as long as the equalities we rewrite are
instantiated with proofs. This is easy to see, because RTT can be
translated to extensional type theory~\cite{}, a theory known to be
consistent. The translation simply turns each rewrite rule reduction
...
at the cost of decidability of type checking. Due to this lack of
decidability, extensional type theory is not a common candidate for
a basis of a proof assistant.
