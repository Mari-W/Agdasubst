...
opaque
  ...
  sub-wk-var : ∀ {Δ}(u : Tm ↑ Δ) → sub var (wkSub (tt ,- u)) ≡ wk↑ (sub var (tt ,- u))
  sub-wk-var u = refl
...
opaque
  ...
  wkSub-thinSub {Γ = []} ψ _ = refl
  wkSub-thinSub {Γ = _ ∷ _} ψ (σ ,- (t ⇑ ξ)) =
...
opaque
  ...
  wkSub≡thin {Γ = []} _ = refl
  wkSub≡thin {Γ = _ ∷ _} (ρ ,- (t ⇑ ξ)) = cong₂ _,-_ (wkSub≡thin ρ) (cong (t ⇑_) (sym (⨾-o' ξ oi)))
...
Ass {Γ = []} _ τ υ = refl
Ass {Γ = _ ∷ _} (σ ,- (t ⇑ θ)) τ υ = cong₂ _,-_ (Ass σ τ υ) (⟪⟫-fusion (t ⇑ θ) τ υ)
...
opaque
  unfolding sub lift
  VarCons-z : ∀ {Δ}(u : Tm ↑ Δ) → sub var (tt ,- u) ≡ u
  VarCons-z u = refl
...
opaque
  ...
  wk-⨟-cons {Γ = []} _ τ u = refl
  wk-⨟-cons {Γ = _ ∷ _} (σ ,- (t ⇑ ξ)) τ u = cong (_,- sub t (τ ↾ ξ)) (wk-⨟-cons σ τ u)
...
opaque
  ...
  wkSub-thinSub-o' {Γ = []} ψ _ = refl
  wkSub-thinSub-o' {Γ = _ ∷ _} ψ (ρ ,- (t ⇑ ξ)) = cong₂ _,-_ (wkSub-thinSub-o' ψ ρ) (cong (t ⇑_) (sym (⨾-o' ξ ψ)))
...
IdR {Γ = []} _ = refl
IdR {Γ = _ ∷ _} (σ ,- u) = cong₂ _,-_ (IdR σ) (⟪⟫-id u)
...
thinSub-∘ {Γ = []} φ ψ _ = refl
thinSub-∘ {Γ = _ ∷ _} φ ψ (ρ ,- (t ⇑ ξ)) = cong₂ _,-_ (thinSub-∘ φ ψ ρ) refl
...
-- ════════════════════════════════════════════════════════════════════════════
↑ₛ : ∀ {Γ} → Sub (tt ∷ Γ) Γ          -- the σ_SP shift primitive `↑`
↑ₛ = wkSub idS
opaque
  unfolding _⟪_⟫ wkSub
