...
opaque
  unfolding _∙ˢ_ _↑ˢ_ _⋯ˢ_ _⋯ˢ_
  wk-beta                 : x ⋯ˢ (wkˢ s)       ≡ ` suc x                                  -- varshift1
  wk-beta = refl

  wk-beta-compˢ           : x ⋯ˢ (wkˢ s ;ˢˢ σ) ≡ suc x ⋯ˢ σ                               -- varshift2s
  wk-beta-compˢ = refl

  ext-beta-zeroˢ          : zero ⋯ˢ (t ∙ˢ σ)           ≡ t                               -- fvarconss
  ext-beta-zeroˢ = refl
  lift-beta-zeroˢ         : zero ⋯ˢ (σ ↑ˢ s)           ≡ ` zero                          -- fvarlift2s
  lift-beta-zeroˢ = refl
  lift-beta-zero-compˢˢ   : zero ⋯ˢ ((σ₁ ↑ˢ s) ;ˢˢ σ₂) ≡ zero ⋯ˢ σ₂                      -- fvarlift2ss
  lift-beta-zero-compˢˢ = refl
  ext-beta-sucˢ           : suc x ⋯ˢ (t ∙ˢ σ)  ≡ x ⋯ˢ σ                                  -- rvarconss 
  ext-beta-sucˢ = refl

  coincidence        : Q ⋯ˢ ((λ s x → ` (ρ _ x)))  ≡ Q ⋯ᴿ ρ

  coincidence-foldᵀ  : (t′ ⋯ˢ ((λ s x → ` (ρ ↑ᴿ type) s x) ;ˢˢ ((t ⋯ᴿ ρ) ∙ˢ (λ s → `_)))) ≡ 
                         ((t′ ⋯ˢ ((t ⋯ᴿ ρ) ∙ˢ (λ s z → ` ρ s z))))

  lift-beta-sucˢ          : suc x ⋯ˢ (σ ↑ˢ s)  ≡ x ⋯ˢ (σ ;ˢˢ wkˢ s)                       -- rvarlift1s
  lift-beta-sucˢ {x = x} = sym coincidence
  ...
  lift-beta-suc-compˢˢ    : suc x ⋯ˢ ((σ₁ ↑ˢ s) ;ˢˢ σ₂) ≡ x ⋯ˢ (σ₁ ;ˢˢ (wkˢ s ;ˢˢ σ₂))    -- rvarlift2ss
  lift-beta-suc-compˢˢ {x = x} {σ₁ = σ₁} {s = s} {σ₂ = σ₂} = compositionalityᴿˢ {Q = σ₁ _ x} {ρ₁ = λ s₁ → suc} {σ₂ = σ₂}
  ...
  wk-liftˢ                : wkˢ s ;ˢˢ (σ ↑ˢ s) ≡ σ ;ˢˢ wkˢ s                               -- shiftlift1s
  wk-lift-compˢˢ          : wkˢ s ;ˢˢ ((σ₁ ↑ˢ s) ;ˢˢ σ₂) ≡ σ₁ ;ˢˢ (wkˢ s  ;ˢˢ σ₂)          -- shiftlift2ss  
