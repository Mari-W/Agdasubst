...
equational theory, renamings are required in proof assistants to model
...
reduction of most of the symbols for primitives, and describe their behavior
later through rewrite rules.
...
symbol for instantiation, instead of function application. Because we need to
state the equations we wish to rewrite later only in terms of stuck symbols,
without using lambdas or function application, we require the one for variable
lookup. We go into more detail in \Cref{sec:mis-ex}.
...
as Agda equalities admissible for rewriting.
...
$η$-law is not instantiable with a proof, because this would require that the
...
To illustrate this, consider the example of classical sets. When reasoning over
sets in Agda, users usually resort to work with lists up to permutation and a
...
rewrite rules, and we expect our approach can be translated to Rocq. However,
Rocq lacks the ability to instantiate the rewrite rules with
proofs~\cite{leray_et_al:LIPIcs.ITP.2024.26}, and we would then lose
